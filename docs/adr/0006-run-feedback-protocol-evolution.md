---
node: adr/0006-run-feedback-protocol-evolution
kind: adr
edges:
  decides:
    - architecture
...
# ADR 0006: Gateway and Feedback Run Protocol Evolution

Status: accepted

## Context

ADR 0005 deliberately froze a bounded Daily Operator baseline grammar without gateway,
outcome, evaluation, or state-transition events. That stream is already durable public evidence
and cannot be reinterpreted in place. Runtime foundation must add a real gateway-backed decision path and
close the feedback loop while preserving exact baseline replay.
...
Keep the persisted `daily-operator/v1` baseline contract immutable and introduce the
`daily-operator/v2` feedback contract. One public replay use case dispatches by that recorded
contract identity. New behavior is added through feature-owned ports and
...
Feedback grammar deliberately supports one attempt because its journal fails closed after an uncertain
interruption. Retries require a new run or future protocol revision. The attempt and terminal events
...
feedback branch, including eventual model failure or denial. `run.initial-state-recorded` is required once
...
feedback grammar does not automatically resume or charge another model call.
...
Freezing this ADR, codecs, and validators does not activate the feedback grammar or change the
current baseline recorder default. A run records its workflow contract at `run.started` and never
changes it mid-stream.

The feedback writer becomes selectable only when EvaluationSpec, initial-state snapshot, gateway
request/attempt/terminal evidence, independent observer, outcome-state snapshot, evaluation,
optional transition, strict feedback validation, and feedback replay verification are composed. The public
facade switches only after live-free replay and compatibility characterization. Blackcell never
emits a gateway-only or partial feedback history.
...
- validates both baseline and feedback grammar, correlation, causation, and artifact ownership;
- verifies cross-artifact identities and recorded state cutoffs;
- reports baseline state artifacts as not recorded rather than failed;
...
- Existing baseline evidence remains stable and replayable.
...
- Mutate the baseline grammar: this would change the meaning of committed event history.
