"""Closed, owner-only runtime configuration for opt-in execution dispatch."""
...
EXECUTION_CONFIG_FILE_ENV = "BLACKCELL_EXECUTION_CONFIG_FILE"
EXECUTION_CONFIG_SCHEMA = "blackcell.execution-worker-config/v3"
...
class ExecutionWorkerConfigFailureCode(StrEnum):
    INVALID = "invalid-execution-worker-config"


class ExecutionWorkerConfigError(RuntimeError):
    """A content-free execution configuration failure."""

    def __init__(self) -> None:
        self.code = ExecutionWorkerConfigFailureCode.INVALID
        super().__init__(self.code.value)


class ExecutionProviderAdapter(StrEnum):
    CODEX_CLI = "codex-cli"
    AGY_CLI = "agy-cli"


@dataclass(frozen=True, slots=True)
class ExecutionProviderRuntimeConfig:
...
@dataclass(frozen=True, slots=True)
class ExecutableAliasConfig:
    alias: str
    path: Path


@dataclass(frozen=True, slots=True)
class IsolationRuntimeConfig:
    root: Path
    executables: tuple[ExecutableAliasConfig, ...]
...
@dataclass(frozen=True, slots=True)
class ExecutionWorkerLoopConfig:
    worker_id: str
...
@dataclass(frozen=True, slots=True)
class ExecutionWorkerRuntimeConfig:
    source_path: Path
    provider: ExecutionProviderRuntimeConfig
    isolation: IsolationRuntimeConfig
    worker: ExecutionWorkerLoopConfig
    schema_version: str = EXECUTION_CONFIG_SCHEMA


def load_execution_worker_config(
    environment: Mapping[str, str],
    *,
    repository_root: Path,
    data_root: Path,
    expected_uid: int | None = None,
) -> ExecutionWorkerRuntimeConfig | None:
    """Load the optional execution worker file without accepting ambient defaults."""

    value = environment.get(EXECUTION_CONFIG_FILE_ENV)
    if value is None:
        return None
    try:
        ...
        if payload.get("schema_version") != EXECUTION_CONFIG_SCHEMA:
        ...
        return ExecutionWorkerRuntimeConfig(source, provider, isolation, worker)
    except ExecutionWorkerConfigError:
        raise
    except (KeyError, OSError, RuntimeError, TypeError, ValueError) as error:
        raise ExecutionWorkerConfigError from error


def _provider(value: Mapping[str, object]) -> ExecutionProviderRuntimeConfig:
    try:
        adapter = ExecutionProviderAdapter(_text(value, "adapter"))
    except ValueError as error:
        raise ExecutionWorkerConfigError from error
    if adapter is ExecutionProviderAdapter.CODEX_CLI:
    ...
    if len(set(environment_variables)) != len(environment_variables):
        raise ExecutionWorkerConfigError
    return ExecutionProviderRuntimeConfig(
        adapter=adapter,
...
def _isolation(
    ...
    *,
    ...
    if not isinstance(aliases_value, Mapping) or not 1 <= len(aliases_value) <= 64:
        raise ExecutionWorkerConfigError
    aliases: list[ExecutableAliasConfig] = []
    for alias, executable in aliases_value.items():
        if (
            ...
            raise ExecutionWorkerConfigError
        aliases.append(ExecutableAliasConfig(alias, _executable(executable)))
