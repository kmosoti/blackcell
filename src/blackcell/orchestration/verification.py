"""Deterministic intent-to-evidence verification for execution review artifacts."""
...
VERIFICATION_REPORT_SCHEMA = "verification-report/v1"
VERIFICATION_MATRIX_SCHEMA = "verification-matrix/v1"
...
class VerificationCriterionKind(StrEnum):
    OBJECTIVE = "objective"
...
class VerificationReasonCode(StrEnum):
    ...
    EPISTEMIC_COVERAGE_COMPLETE = "epistemic-coverage-complete"
    EPISTEMIC_NOT_APPLICABLE = "epistemic-not-applicable"
    EPISTEMIC_CONCERN = "epistemic-concern"
    EPISTEMIC_UNKNOWN = "epistemic-unknown"


class VerificationFailureCode(StrEnum):
    INVALID_INPUT = "invalid-verification-input"
    BINDING_MISMATCH = "verification-binding-mismatch"
    CITATION_MISMATCH = "verification-citation-mismatch"


class VerificationError(ValueError):
    """Content-free rejection at the deterministic verifier boundary."""

    def __init__(self, code: VerificationFailureCode) -> None:
        self.code = code
...
class VerificationMatrixRow:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            or not isinstance(self.kind, VerificationCriterionKind)
            or not isinstance(self.claim_digest, str)
            or _DIGEST.fullmatch(self.claim_digest) is None
            or not isinstance(self.status, VerificationStatus)
            or not isinstance(self.reason_codes, tuple)
            or not self.reason_codes
            or not all(isinstance(item, VerificationReasonCode) for item in self.reason_codes)
            ...
            raise VerificationError(VerificationFailureCode.INVALID_INPUT)
        ...
        if node_kind != (self.node_id is not None) or (
            self.kind is VerificationCriterionKind.CHECK
        ) != (self.check_id is not None):
            raise VerificationError(VerificationFailureCode.INVALID_INPUT)
        ...
        unresolved = VerificationReasonCode.UNRESOLVED_REVIEW_FINDING in reasons
        epistemic_concern = VerificationReasonCode.EPISTEMIC_CONCERN in reasons
        epistemic_reasons = {
        ...
        selected_epistemic = set(reasons) & epistemic_reasons
        if (
            (unresolved or epistemic_concern) != bool(findings)
            or (VerificationReasonCode.EVIDENCE_COMPLETE in reasons and len(reasons) != 1)
            or (
                VerificationReasonCode.REVIEW_CLEAR in reasons
                and (
                    len(reasons) != 1
                    or self.kind is not VerificationCriterionKind.REVIEW_POLICY
                    or findings
                )
            )
            or (
                self.kind is VerificationCriterionKind.REVIEW_POLICY
                and not unresolved
                and VerificationReasonCode.REVIEW_CLEAR not in reasons
            )
            or (
                self.kind is VerificationCriterionKind.EPISTEMIC_POLICY
                and (len(reasons) != 1 or len(selected_epistemic) != 1)
            )
            or (self.kind is not VerificationCriterionKind.EPISTEMIC_POLICY and selected_epistemic)
        ):
            raise VerificationError(VerificationFailureCode.INVALID_INPUT)
...
class VerificationReport:
    ...
    status: VerificationStatus
    matrix: tuple[VerificationMatrixRow, ...]
    ...
    def __post_init__(self) -> None:
        ...
        if (
            self.schema_version != VERIFICATION_REPORT_SCHEMA
            ...
            or not all(isinstance(item, VerificationMatrixRow) for item in self.matrix)
            or len({item.criterion_id for item in self.matrix}) != len(self.matrix)
            or self.status is not _aggregate_status(self.matrix)
        ):
            raise VerificationError(VerificationFailureCode.INVALID_INPUT)
        if len(canonical_json_bytes(verification_report_payload(self))) > _MAX_REPORT_BYTES:
            raise VerificationError(VerificationFailureCode.INVALID_INPUT)

    @property
    def digest(self) -> str:
        return json_digest(verification_report_payload(self))

    @property
    def matrix_digest(self) -> str:
        return json_digest(verification_matrix_payload(self))


def verify_review(
    context: ReviewContext,
    admitted: AdmittedReview,
) -> VerificationReport:
    """Build a host-owned verdict without reinterpreting acceptance or reviewer prose."""

    if not isinstance(context, ReviewContext) or not isinstance(admitted, AdmittedReview):
        raise VerificationError(VerificationFailureCode.INVALID_INPUT)
    if (
    ...
    _validate_citations(
        admitted.findings,
        admitted.epistemic_assessments,
        evidence_by_id,
    )
    index = _EvidenceIndex(context.evidence)
    finding_index = _FindingIndex(admitted.findings, evidence_by_id)

    node_rows: list[VerificationMatrixRow] = []
    supporting_rows: list[VerificationMatrixRow] = []
    ...
    epistemic_rows = tuple(_epistemic_row(item) for item in admitted.epistemic_assessments)
...
def verification_report_payload(value: VerificationReport) -> dict[str, JsonInput]:
    if not isinstance(value, VerificationReport):
        raise VerificationError(VerificationFailureCode.INVALID_INPUT)
    return {
...
def verification_matrix_payload(value: VerificationReport) -> dict[str, JsonInput]:
    if not isinstance(value, VerificationReport):
        raise VerificationError(VerificationFailureCode.INVALID_INPUT)
    return {
        "schema_version": VERIFICATION_MATRIX_SCHEMA,
        "rows": [_matrix_row_payload(row) for row in value.matrix],
    }


def verification_report_from_mapping(
    ...
    if (
        ...
        or raw.get("schema_version") != VERIFICATION_REPORT_SCHEMA
    ):
        raise VerificationError(VerificationFailureCode.INVALID_INPUT)
    try:
        status = VerificationStatus(raw.get("status"))
    except TypeError, ValueError:
        raise VerificationError(VerificationFailureCode.INVALID_INPUT) from None
    matrix = tuple(
...
def _matrix_row_payload(row: VerificationMatrixRow) -> dict[str, JsonInput]:
    return {
...
def _matrix_row_from_mapping(value: object) -> VerificationMatrixRow:
    raw = _mapping(value)
    if set(raw) != {
        "criterion_id",
        "kind",
        ...
        raise VerificationError(VerificationFailureCode.INVALID_INPUT)
    try:
        kind = VerificationCriterionKind(raw.get("kind"))
        status = VerificationStatus(raw.get("status"))
        reasons = tuple(
            VerificationReasonCode(item)
            for item in _sequence(
                raw.get("reason_codes"),
                minimum=1,
                maximum=len(VerificationReasonCode),
            )
        )
    except TypeError, ValueError:
        raise VerificationError(VerificationFailureCode.INVALID_INPUT) from None
    return VerificationMatrixRow(
...
class _FindingIndex:
    ...
    evidence_by_id: Mapping[str, ReviewEvidence]

    def for_node(self, node_id: str) -> tuple[ProposedReviewFinding, ...]:
        return tuple(
            finding
    ...
    def for_check(self, node_id: str, check_id: str) -> tuple[ProposedReviewFinding, ...]:
        return tuple(
            finding
            for finding in self.findings
            if any(
    ...
    def scope_for_node(self, node_id: str) -> tuple[ProposedReviewFinding, ...]:
        return tuple(
            finding
            for finding in self.for_node(node_id)
            if finding.category is ReviewFindingCategory.SCOPE_DRIFT
        )
...
def _check_row(
    ...
    if any(count == 0 for count in counts):
        reasons.append(VerificationReasonCode.CHECK_EVIDENCE_MISSING)
    if any(count > 1 for count in counts):
        reasons.append(VerificationReasonCode.EVIDENCE_AMBIGUOUS)
    ...
    if (len(commands) == 1 and commands[0].artifact_digest != check.command_digest) or (
        ...
        reasons.append(VerificationReasonCode.EVIDENCE_IDENTITY_MISMATCH)
    if not check.passed:
        reasons.append(VerificationReasonCode.CHECK_FAILED)
    if matched_findings:
        reasons.append(VerificationReasonCode.UNRESOLVED_REVIEW_FINDING)
    if not reasons:
        reasons.append(VerificationReasonCode.EVIDENCE_COMPLETE)
    return _row(
        criterion_id=f"check-{node_number:03d}-{check_number:03d}",
        kind=VerificationCriterionKind.CHECK,
        claim={
            "kind": "check",
            "node_id": node.node_id,
            "check_id": check.check_id,
...
def _scope_row(
    ...
    if not paths:
        reasons.append(VerificationReasonCode.CHANGE_EVIDENCE_MISSING)
    for path in paths:
        ...
        if len(operations) != 1 or None in operations:
            reasons.append(VerificationReasonCode.EVIDENCE_AMBIGUOUS)
            continue
        operation = cast("TextOperation", next(iter(operations)))
        required = {
            TextOperation.CREATE: (
                ReviewEvidenceKind.SOURCE_AFTER,
                ReviewEvidenceKind.EFFECT,
            ),
            TextOperation.REPLACE: change_kinds,
    ...
    return _row(
        ...
        kind=VerificationCriterionKind.WRITE_SCOPE,
        claim={
            "kind": "write-scope",
            "node_id": node.node_id,
...
def _node_row(
    ...
    findings: _FindingIndex,
    *,
    scope_row: VerificationMatrixRow | None,
    check_rows: tuple[VerificationMatrixRow, ...],
) -> VerificationMatrixRow:
    outcomes = evidence.select(ReviewEvidenceKind.OUTCOME, node_id=node.node_id)
    matched_findings = findings.for_node(node.node_id)
    reasons: list[VerificationReasonCode] = []
    if not outcomes:
        reasons.append(VerificationReasonCode.NODE_OUTCOME_MISSING)
    elif len(outcomes) > 1:
        reasons.append(VerificationReasonCode.EVIDENCE_AMBIGUOUS)
    ...
    for row in child_rows:
        reasons.extend(
            ...
            if reason is not VerificationReasonCode.EVIDENCE_COMPLETE
        )
    if matched_findings:
        reasons.append(VerificationReasonCode.UNRESOLVED_REVIEW_FINDING)
    if not reasons:
    ...
    return _row(
        ...
        kind=VerificationCriterionKind.NODE,
        claim={
            "kind": "node",
            "node_id": node.node_id,
...
def _intent_row(
    ...
    node_rows: tuple[VerificationMatrixRow, ...],
    findings: tuple[ProposedReviewFinding, ...],
) -> VerificationMatrixRow:
    reasons = [
        reason
...
def _review_row(
    ...
    reasons = (
        (VerificationReasonCode.UNRESOLVED_REVIEW_FINDING,)
        if admitted.findings
        else (VerificationReasonCode.REVIEW_CLEAR,)
    )
    cited_ids = tuple(
    ...
    return VerificationMatrixRow(
        criterion_id="review",
        kind=VerificationCriterionKind.REVIEW_POLICY,
...
def _epistemic_row(assessment: EpistemicAssessment) -> VerificationMatrixRow:
    reason = {
        EpistemicDisposition.SUPPORTED: (VerificationReasonCode.EPISTEMIC_COVERAGE_COMPLETE),
        EpistemicDisposition.NOT_APPLICABLE: (VerificationReasonCode.EPISTEMIC_NOT_APPLICABLE),
        EpistemicDisposition.CONCERN: VerificationReasonCode.EPISTEMIC_CONCERN,
        EpistemicDisposition.UNKNOWN: VerificationReasonCode.EPISTEMIC_UNKNOWN,
    ...
    return VerificationMatrixRow(
        criterion_id=f"epistemic-{assessment.dimension.value}",
        kind=VerificationCriterionKind.EPISTEMIC_POLICY,
        claim_digest=json_digest(
            {
                "kind": "epistemic-policy",
        ...
        status=_status_for_reasons((reason,)),
        reason_codes=(reason,),
        evidence_ids=tuple(item.evidence_id for item in assessment.citations),
...
def _validate_citations(
    ...
    citations = (
        ...
        *(citation for item in assessments for citation in item.citations),
    )
    for citation in citations:
        ...
        if (
            ...
            raise VerificationError(VerificationFailureCode.CITATION_MISMATCH)
...
def _status_for_reasons(
    ...
    if any(
        ...
        in {
            VerificationReasonCode.UNRESOLVED_REVIEW_FINDING,
            VerificationReasonCode.EVIDENCE_IDENTITY_MISMATCH,
    ...
    if any(
        reason
        in {
            VerificationReasonCode.NODE_OUTCOME_MISSING,
            VerificationReasonCode.EVIDENCE_AMBIGUOUS,
            VerificationReasonCode.CHECK_EVIDENCE_MISSING,
            VerificationReasonCode.CHANGE_EVIDENCE_MISSING,
            VerificationReasonCode.EPISTEMIC_UNKNOWN,
        }
        for reason in reasons
    ):
...
def _aggregate_status(
    ...
    if any(row.status is VerificationStatus.FAIL for row in matrix):
        return VerificationStatus.FAIL
    if any(row.status is VerificationStatus.INCONCLUSIVE for row in matrix):
        return VerificationStatus.INCONCLUSIVE
    return VerificationStatus.PASS
