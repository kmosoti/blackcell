...
const MAX_RESPONSE_BYTES = 256 * MAX_REQUEST_BYTES;
...
class RuntimeApiClient {
  ...
  async issueSocketTicket() {
    const value = await this._request("/api/v1/ui/socket-tickets", { method: "POST" });
    if (
      !isObject(value) ||
  ...
  async inspectRun(runId) {
    ...
    const value = await this._request(
      `/api/v1/runs/${encodeURIComponent(expected)}/status`,
    );
  ...
  async replayRun(runId) {
    const expected = validRunId(runId);
    const value = await this._request(
      `/api/v1/runs/${encodeURIComponent(expected)}/replay`,
    );
  ...
  async cancelRun(runId) {
    ...
    const value = await this._request(
      `/api/v1/runs/${encodeURIComponent(expected)}/cancel`,
      { method: "POST", body },
    );
    return validateRunResponse(value, expected);
  }
...
function validateEventPage(page, expectedAfter) {
  if (
    ...
    page.schema_version !== "event-page/v1" ||
...
function validateProjectRequest(value) {
  if (
    ...
    value.schema_version !== "project-request/v1" ||
...
function validateIntentRequest(value) {
  if (
    ...
    value.schema_version !== "intent-request/v1" ||
...
function validatePlanRequest(value) {
  if (
    ...
    value.schema_version !== "plan-request/v1" ||
...
function validateRunRequest(value) {
  if (
    ...
    value.schema_version !== "run-request/v1" ||
...
function validateProjectResponse(value, request) {
  if (
    ...
    value.schema_version !== "project/v1" ||
...
function validateIntentResponse(value, request) {
  if (
    ...
    value.schema_version !== "intent/v1" ||
...
function validateReplayResponse(value, expectedRunId) {
  if (
    ...
    value.schema_version !== "replay/v2" ||
    value.run_id !== expectedRunId ||
    !isObject(value.project) ||
    value.project.schema_version !== "project/v1" ||
    !isObject(value.intent) ||
    value.intent.schema_version !== "intent/v1" ||
    !isObject(value.plan) ||
    value.plan.schema_version !== "plan/v1" ||
    !isObject(value.run) ||
    ...
    value.verification.schema_version !== "verification-replay/v1"
  ) {
