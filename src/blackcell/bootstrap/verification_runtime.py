...
class VerificationRuntimeFailureCode(StrEnum):
    INVALID_REQUEST = "invalid-verification-runtime-request"
    NOT_FOUND = "verification-runtime-not-found"
    CONFLICT = "verification-runtime-conflict"


class VerificationRuntimeError(RuntimeError):
    """Content-free scheduler failure for deterministic verification work."""

    def __init__(self, code: VerificationRuntimeFailureCode) -> None:
        self.code = code
        super().__init__(code.value)


@dataclass(frozen=True, slots=True)
class ClaimedVerification:
    lease: VerificationLease
    claim_event_id: str


@dataclass(frozen=True, slots=True)
class VerificationReconciliationReport:
...
class VerificationRuntimeService:
    ...
    def inspect(self, run_id: str) -> VerificationLifecycleState | None:
        try:
            stream = self.events.read_stream(verification_stream(run_id))
            return None if not stream else fold_verification_lifecycle(run_id, stream)
        except VerificationLifecycleError as error:
            raise VerificationRuntimeError(VerificationRuntimeFailureCode.CONFLICT) from error
    ...
    def claim(
        ...
        *,
        ...
        if not isinstance(candidate, VerificationCandidate):
            raise VerificationRuntimeError(VerificationRuntimeFailureCode.INVALID_REQUEST)
        try:
            ...
            raise VerificationRuntimeError(
                VerificationRuntimeFailureCode.INVALID_REQUEST
            ) from error
        if expires_at <= at:
            raise VerificationRuntimeError(VerificationRuntimeFailureCode.INVALID_REQUEST)
        ...
        stream_id = verification_stream(candidate.run_id)
        existing = self.events.read_stream(stream_id)
        previous: VerificationLifecycleState | None = None
        if existing:
            try:
                previous = fold_verification_lifecycle(candidate.run_id, existing)
            except VerificationLifecycleError as error:
                raise VerificationRuntimeError(VerificationRuntimeFailureCode.CONFLICT) from error
            if previous.status is not VerificationLifecycleStatus.REQUEUED:
                raise VerificationRuntimeError(VerificationRuntimeFailureCode.CONFLICT)
        ...
        return ClaimedVerification(lease=lease, claim_event_id=event.event_id)
    ...
    def reconcile(self, *, principal_id: str) -> VerificationReconciliationReport:
        ...
        for run_id in self._verification_run_ids():
            ...
            if state.status is not VerificationLifecycleStatus.CLAIMED:
                continue
            if principal == state.lease.worker_id:
                raise VerificationRuntimeError(VerificationRuntimeFailureCode.CONFLICT)
            existing = self.events.read_stream(verification_stream(run_id))
            event = self._transition(
                state,
                VERIFICATION_REQUEUED,
                {
        ...
        return VerificationReconciliationReport(tuple(requeued))

    def _require_active(
        self,
        lease: VerificationLease,
        principal_id: str,
    ) -> tuple[VerificationLifecycleState, tuple[EventEnvelope, ...]]:
        if not isinstance(lease, VerificationLease):
            raise VerificationRuntimeError(VerificationRuntimeFailureCode.INVALID_REQUEST)
        try:
            principal = _identifier(principal_id)
        except ValueError as error:
        ...
        existing = self.events.read_stream(verification_stream(lease.run_id))
        if not existing:
            raise VerificationRuntimeError(VerificationRuntimeFailureCode.NOT_FOUND)
        try:
            state = fold_verification_lifecycle(lease.run_id, existing)
        except VerificationLifecycleError as error:
            raise VerificationRuntimeError(VerificationRuntimeFailureCode.CONFLICT) from error
        if (
            state.status is not VerificationLifecycleStatus.CLAIMED
    ...
    def _require_state(self, run_id: str) -> VerificationLifecycleState:
    ...
    def _require_source_events(
        ...
        if (
            execution is None
            or execution.stream_id != f"run:{candidate.run_id}"
            or execution.event_type != RUN_SUCCEEDED
            or execution.source != RUNTIME_EVENT_SOURCE
            ...
            or review is None
            or review.stream_id != review_stream(candidate.run_id)
            or review.event_type != REVIEW_SUCCEEDED
            or review.source != RUNTIME_EVENT_SOURCE
            or review.correlation_id != candidate.correlation_id
            or review.payload_hash != candidate.review_event_digest
        ):
            raise VerificationRuntimeError(VerificationRuntimeFailureCode.CONFLICT)
        try:
            review_events = self.events.read_stream(review_stream(candidate.run_id))
            review_state = fold_review_lifecycle(candidate.run_id, review_events)
        except ReviewLifecycleError as error:
            raise VerificationRuntimeError(VerificationRuntimeFailureCode.CONFLICT) from error
        lease = review_state.lease
        if (
            review_state.status is not ReviewLifecycleStatus.SUCCEEDED
            ...
            raise VerificationRuntimeError(VerificationRuntimeFailureCode.CONFLICT)
        return execution, review, frozenset(event.actor for event in review_events)
    ...
    def _transition(
        ...
        *,
        ...
        return EventEnvelope.create(
            stream_id=verification_stream(state.run_id),
    ...
    def _append(
        ...
        try:
            fold_verification_lifecycle(run_id, (*existing, event))
            stored = self.events.append(event, expected_sequence=len(existing))
            fold_verification_lifecycle(run_id, (*existing, stored))
        except VerificationLifecycleError as error:
            raise VerificationRuntimeError(VerificationRuntimeFailureCode.CONFLICT) from error
    ...
    def _verification_run_ids(self) -> tuple[str, ...]:
        ...
        while True:
            ...
            if not events:
                break
            for event in events:
                if event.event_type == VERIFICATION_CLAIMED and event.stream_id.startswith(
                    "verification:"
                ):
                    run_id = event.stream_id.removeprefix("verification:")
