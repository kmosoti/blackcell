"""Bootstrap coordinator joining execution runtime, provider, effects, isolation, and artifacts."""
...
from blackcell.bootstrap.runtime_service import PreparedNode, ReadyNode
...
from blackcell.interfaces.http import RunResponse, RuntimeApiError
...
MAX_EXECUTION_READ_BASE_WORKTREE_SECONDS = GIT_WORKTREE_COMMAND_TIMEOUT_SECONDS * (
    MAX_GIT_WORKTREE_CREATE_COMMANDS
    + MAX_GIT_WORKTREE_COMMIT_COMMANDS
    + MAX_GIT_WORKTREE_INSPECT_COMMANDS
)
MAX_EXECUTION_WRITE_BASE_WORKTREE_SECONDS = (
    MAX_EXECUTION_READ_BASE_WORKTREE_SECONDS
    + 3 * GIT_WORKTREE_COMMAND_TIMEOUT_SECONDS * MAX_GIT_WORKTREE_INSPECT_COMMANDS
)
MAX_EXECUTION_ACCEPTANCE_WORKTREE_SECONDS = (
    2 * GIT_WORKTREE_COMMAND_TIMEOUT_SECONDS * MAX_GIT_WORKTREE_INSPECT_COMMANDS
)


class ExecutionWorkerFailureCode(StrEnum):
    CHECK_FAILED = "execution-acceptance-check-failed"
    INVALID_ACCEPTANCE_RESULT = "invalid-execution-acceptance-result"
    PROVIDER_FAILED = "change-provider-failed"
    ARTIFACT_FAILED = "execution-worker-artifact-failed"
    UNEXPECTED = "execution-worker-unexpected-failure"


class ExecutionWorkerError(RuntimeError):
    """A stable worker failure that never includes provider, project, or stream content."""

    def __init__(self, code: ExecutionWorkerFailureCode) -> None:
        self.code = code
        super().__init__(code.value)


class ChangeProviderPort(Protocol):
    def propose(self, call: ChangeProviderCall) -> ChangeProviderResult: ...


class AcceptanceRunnerPort(Protocol):
    def run(
        self,
...
class ExecutionWorkerPolicy:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError("invalid execution worker policy")
...
class ExecutionWorker:
    runtime: ExecutionWorkerPort
    artifacts: ExecutionArtifactStorePort
    provider: ChangeProviderPort
    change_executor: TextChangeExecutor
    acceptance: AcceptanceRunnerPort
    worktrees: GitWorktreeLifecycle = field(default_factory=GitWorktreeLifecycle, repr=False)
    evidence: ExecutionEvidenceCollector = field(
        default_factory=ExecutionEvidenceCollector, repr=False
    )
    policy: ExecutionWorkerPolicy = field(
        default_factory=lambda: ExecutionWorkerPolicy("execution-worker")
    )

    def run_once(self) -> ExecutionWorkerCycleResult:
        ready = self.runtime.next_ready_node()
        if ready is None:
            return ExecutionWorkerCycleResult(status="idle")
        ...
        base_worktree_seconds = (
            ...
            else MAX_EXECUTION_READ_BASE_WORKTREE_SECONDS
        )
        worktree_seconds = (
            base_worktree_seconds
            + acceptance_check_count * MAX_EXECUTION_ACCEPTANCE_WORKTREE_SECONDS
        )
    ...
    def _execute(self, prepared: PreparedNode) -> ExecutionWorkerCycleResult:
        ...
        try:
            ...
            for check in prepared.node.checks:
                ...
                if not result.passed:
                    raise ExecutionWorkerError(ExecutionWorkerFailureCode.CHECK_FAILED)
            self._require_active(spec)
            ...
            try:
                ...
                failure_code = ExecutionWorkerFailureCode.ARTIFACT_FAILED.value
    ...
    def _require_active(self, spec: WorktreeExecutionSpec) -> None:
        if self.runtime.should_cancel_node(spec):
            raise AcceptanceError(AcceptanceFailureCode.CANCELED)

    def _store_effect(self, result: TextChangeExecutionResult) -> ExecutionArtifactLink:
        ...
        *,
        ...
        if status == "succeeded":
            if _COMMIT.fullmatch(head_commit or "") is None or failure_code is not None:
                raise ExecutionWorkerError(ExecutionWorkerFailureCode.ARTIFACT_FAILED)
    ...
    def _store_json(
        ...
        *,
        ...
        try:
            ...
            raise ExecutionWorkerError(ExecutionWorkerFailureCode.ARTIFACT_FAILED) from error
    ...
    def _validate_acceptance_result(
        ...
        if (
            ...
            raise ExecutionWorkerError(ExecutionWorkerFailureCode.INVALID_ACCEPTANCE_RESULT)
...
def _failure_code(error: Exception, *, phase: str) -> str:
    ...
    if isinstance(error, AcceptanceError | WorktreeLifecycleError):
        return error.code.value
    if isinstance(error, ChangeContractError):
        return error.code.value
    if isinstance(error, GatewayAdmissionError):
        return error.code.value
    if isinstance(error, KernelError | OSError):
        return ExecutionWorkerFailureCode.ARTIFACT_FAILED.value
    if phase == "provider":
        return ExecutionWorkerFailureCode.PROVIDER_FAILED.value
    return ExecutionWorkerFailureCode.UNEXPECTED.value
...
__all__ = [
    ...
    "MAX_EXECUTION_WRITE_BASE_WORKTREE_SECONDS",
    "NODE_OUTCOME_SCHEMA",
    "AcceptanceRunnerPort",
    "ChangeProviderPort",
    "CheckArtifacts",
    "ExecutionArtifactLink",
    "ExecutionArtifactStorePort",
    "ExecutionWorker",
    "ExecutionWorkerCycleResult",
    "ExecutionWorkerError",
    "ExecutionWorkerFailureCode",
