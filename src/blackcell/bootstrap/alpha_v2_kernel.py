"""Production composition for the durable alpha-v2 repository execution path."""
...
ALPHA_V2_ATTEMPT_MEDIA_TYPE = "application/vnd.blackcell.alpha-v2-attempt+json"
ALPHA_V2_EVIDENCE_MEDIA_TYPE = "application/vnd.blackcell.alpha-v2-evidence+json"


class AlphaV2ArtifactStorePort(Protocol):
    def put_bytes(
        self,
        data: bytes,
        *,
...
class AlphaV2AcceptancePort(Protocol):
    def run(
        self,
...
class AlphaV2ExecutionPolicy:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError("invalid alpha-v2 execution policy")
...
class AlphaV2ExecutionError(RuntimeError):
    """A stable content-free failure at the concrete attempt boundary."""

    def __init__(self, code: str = "alpha-v2-execution-failed") -> None:
        self.code = code
        super().__init__(code)


@dataclass(frozen=True, slots=True)
class ProductionAlphaV2AttemptExecutor:
    """Execute admitted tasks through real worktree, proposal, effect, and sandbox ports."""

    repository_root: Path
    isolation_root: Path
    artifacts: AlphaV2ArtifactStorePort
    change_provider: AlphaV2ChangeProviderPort
    acceptance: AlphaV2AcceptancePort
    policy: AlphaV2ExecutionPolicy
    worktrees: GitWorktreeLifecycle = field(default_factory=GitWorktreeLifecycle, repr=False)
    evidence: AlphaEvidenceCollector | None = field(default=None, repr=False)
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.repository_root, Path) or not isinstance(self.isolation_root, Path):
            raise ValueError("invalid alpha-v2 execution roots")
        ...
        if self.evidence is None:
            object.__setattr__(self, "evidence", AlphaEvidenceCollector(self.worktrees))
    ...
    def execute(
        ...
        *,
        ...
        if not policy_decision.allowed or policy_decision.action_digest != expected_action.digest:
            raise AlphaV2ExecutionError("alpha-v2-policy-denied")
        ...
        try:
            ...
            if task.allowed_paths:
                ...
                context_ref = self._store_json(
                    ...
                    media_type="application/vnd.blackcell.alpha-context+json",
                ...
                if _usage_overdraws_budget(provider_result, call_budget):
                    raise AlphaV2ExecutionError("alpha-v2-cumulative-budget-exhausted")
                proposal_ref = self._store_json(
                    alpha_change_proposal_payload(provider_result.proposal),
                    media_type="application/vnd.blackcell.alpha-proposal+json",
                ...
                provider_ref = self._store_json(
                    ...
                    media_type="application/vnd.blackcell.alpha-provider+json",
                )
                ...
                effect_ref = self._store_json(
                    ...
                    media_type="application/vnd.blackcell.alpha-effect+json",
            ...
            failed: list[AlphaAcceptanceResult] = []
            for check in task.checks:
                ...
                command_ref = self._store_json(
                    ...
                    media_type="application/vnd.blackcell.alpha-check-command+json",
                    expected_digest=command.digest,
                )
                result_ref = self._store_json(
                    alpha_acceptance_result_payload(result),
                    media_type="application/vnd.blackcell.alpha-check-result+json",
            ...
            if not clean.clean or not clean.path_policy_compliant:
                return self._failure(
                    ...
                    error=AlphaV2ExecutionError("alpha-v2-workspace-not-clean"),
                )
            if failed:
            ...
            raise AlphaV2ExecutionError("alpha-v2-artifact-or-io-failed") from error
    ...
    def _failure(
        ...
        *,
        ...
        try:
            ...
            raise AlphaV2ExecutionError(
                "alpha-v2-workspace-evidence-unavailable"
    ...
    def _finish(
        ...
        *,
        ...
        semantic_payload: dict[str, JsonInput] = {
            "schema_version": "blackcell.alpha-v2-evidence/v1",
        ...
        semantic_ref = self._store_json(
            semantic_payload,
            media_type=ALPHA_V2_EVIDENCE_MEDIA_TYPE,
        )
        artifacts.append(semantic_ref.digest)
        record_ref = self._store_json(
            {
                "schema_version": "blackcell.alpha-v2-attempt/v1",
                "run_id": run_id,
                "plan_id": plan.plan_id,
            ...
            media_type=ALPHA_V2_ATTEMPT_MEDIA_TYPE,
        )
    ...
    def _store_json(
        ...
        *,
        ...
        if expected_digest is not None and reference.digest != expected_digest:
            raise AlphaV2ExecutionError("alpha-v2-artifact-digest-mismatch")
        return reference

    def _evidence(self) -> AlphaEvidenceCollector:
    ...
    def _require_not_canceled(self, run_id: str) -> None:
        if self._is_canceled(run_id):
            raise AlphaV2ExecutionError("alpha-v2-canceled")


@dataclass(frozen=True, slots=True)
class AlphaV2KernelRunResult:
    plan: AlphaPlanVersion
    planning: AlphaPlanningResult
    state: AlphaV2RunState
    promotion_candidate: PraxisPromotionCandidate | None


@dataclass(frozen=True, slots=True)
class ProductionAlphaV2Kernel:
    """Application service that consumes the coordinator in the production process graph."""

    coordinator: AlphaV2Coordinator

    def process(self, request: AlphaPlanningRequest, *, actor: str) -> AlphaV2RunState:
        ...
        try:
            ...
            if error.code != "alpha-v2-run-not-found":
                raise
...
def _validate_check(
    ...
    if (
        ...
        raise AlphaV2ExecutionError("invalid-alpha-v2-acceptance-result")


def _classify_failure(error: Exception) -> tuple[FailureClass, str]:
    code = _error_code(error)
    if isinstance(
        error, (AlphaChangeProviderError, AlphaChangeContractError, TextChangeExecutionError)
    ):
        return FailureClass.CONTRACT_MISMATCH, code
    if isinstance(error, AlphaEvidenceError):
        return FailureClass.INVALID_ASSUMPTION, code
    if isinstance(error, GatewayAdmissionError):
        return FailureClass.POLICY, code
    if isinstance(error, AlphaAcceptanceError):
        if error.code in {
            AlphaAcceptanceFailureCode.INVALID_COMMAND,
            AlphaAcceptanceFailureCode.INVALID_POLICY,
            AlphaAcceptanceFailureCode.EXECUTABLE_NOT_ALLOWED,
            AlphaAcceptanceFailureCode.WORKTREE_POLICY_VIOLATION,
        }:
            return FailureClass.POLICY, code
        return FailureClass.TRANSIENT, code
    if isinstance(error, WorktreeLifecycleError):
        return FailureClass.POLICY, code
    if isinstance(error, AlphaV2ExecutionError):
        return FailureClass.POLICY, code
...
def _error_code(error: Exception) -> str:
    ...
    if isinstance(text, str) and text and len(text) <= 128:
        return text
    return "alpha-v2-unclassified-failure"


__all__ = [
    "ALPHA_V2_ATTEMPT_MEDIA_TYPE",
    "ALPHA_V2_EVIDENCE_MEDIA_TYPE",
