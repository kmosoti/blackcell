"""Gateway-backed, proposal-only planner for the alpha-v2 compiler."""
...
class GatewayAlphaPlanner:
    ...
    def propose_plan(self, request: AlphaPlanningRequest) -> AlphaPlanningResult:
        if not isinstance(request, AlphaPlanningRequest):
        ...
        gateway_request = ModelRequest(
            ...
            input=cast("dict[str, JsonValue]", alpha_planning_payload(request.goal)),
            output_schema=cast("dict[str, JsonValue]", ALPHA_PLAN_DRAFT_OUTPUT_SCHEMA),
        ...
        if (
            ...
            raise AlphaV2ContractError("invalid-planning-gateway-result")
...
__all__ = ["GatewayAlphaPlanner", "GatewayInvoker"]
