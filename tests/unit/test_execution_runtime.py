...
from blackcell.bootstrap.execution_plan import ProductionExecution
...
def test_semantic_kernel_symbols_preserve_durable_wire_tokens() -> None:
    assert {
        "attempt_evidence": EXECUTION_ATTEMPT_EVIDENCE_SCHEMA,
        "event_source": EXECUTION_EVENT_SOURCE,
        "goal": EXECUTION_GOAL_SCHEMA,
        "goal_admitted": EXECUTION_GOAL_ADMITTED,
        "plan": EXECUTION_PLAN_SCHEMA,
        "plan_admitted": EXECUTION_PLAN_ADMITTED,
        "plan_draft": EXECUTION_PLAN_DRAFT_SCHEMA,
        "promotion": EXECUTION_PROMOTION_CANDIDATE_SCHEMA,
        "task_verified": EXECUTION_TASK_VERIFIED,
    } == {
        "attempt_evidence": "blackcell.attempt-evidence/v1",
        "event_source": "blackcell.execution",
        "goal": "blackcell.goal/v2",
        "goal_admitted": "execution.goal.admitted",
        "plan": "blackcell.plan/v2",
        "plan_admitted": "execution.plan.admitted",
        "plan_draft": "blackcell.plan-draft/v1",
        "promotion": "praxis-promotion-candidate/v1",
        "task_verified": "execution.task.verified",
    }


def test_plan_compiler_is_deterministic_compositional_and_schema_bound() -> None:
    ...
    first = compile_plan(goal, draft)
    repeated = compile_plan(goal, draft)
    revised = compile_plan(
        goal,
    ...
    assert plan_payload(first)["plan_version"] == 1
    assert "plan_revision" not in plan_payload(first)
...
def test_plan_compiler_rejects_provider_scope_escalation_cycles_and_parallel_writers() -> None:
    ...
    with pytest.raises(PlanContractError, match="plan-path-outside-goal"):
        compile_plan(goal, _draft(tasks=(_task("escape", allowed_paths=("secrets",)),)))
    ...
    with pytest.raises(PlanContractError, match="cyclic-plan"):
        compile_plan(goal, cyclic)
    ...
    with pytest.raises(PlanContractError, match="parallel-writer-plan"):
        compile_plan(goal, parallel)
...
def test_gateway_planner_returns_only_an_untrusted_draft() -> None:
    ...
    goal = replace(
        _goal(),
        verification_checks=(VerificationCheck("check", ("pytest", "-q", secret_command_token)),),
    )
    ...
    assert result.draft["schema_version"] == EXECUTION_PLAN_DRAFT_SCHEMA
...
def test_runtime_executes_one_bounded_repair_then_succeeds_from_snapshot_tail(
    ...
    coordinator = ExecutionCoordinator(journal, provider, executor, ExecutionPolicyKernel())
    ...
    start_positions = [
        item.stream_sequence for item in events if item.event_type == EXECUTION_TASK_STARTED
    ]
...
def test_public_runtime_and_execution_kernel_share_one_canonical_run_stream(
    tmp_path: Path,
) -> None:
    ...
    assert {item.stream_id for item in stream} == {f"run:{run_request.run_id}"}
    assert {item.stream_id for item in events.read_all() if item.stream_id.startswith("run:")} == {
        f"run:{run_request.run_id}"
    }
    ...
    canceled = runtime.cancel_run(
        ...
        CancelRunRequest(
            ...
            idempotency_key="cancel-revised-run",
        ),
    ...
    assert all(
        not item.event_type.startswith("execution.")
...
def test_public_cancellation_during_attempt_stops_verification_and_terminates(
    ...
    assert not any(
        event.event_type in {EXECUTION_TASK_VERIFYING, EXECUTION_TASK_VERIFIED}
...
def test_journal_rejects_invalid_transition_before_append_and_retries_idempotently(
    ...
    with pytest.raises(ExecutionRuntimeError, match="task-not-found"):
        journal.append(
            "run-safe",
            EXECUTION_TASK_STARTED,
            {
...
def test_journal_rejects_unbound_policy_and_attempt_events_before_append(
    ...
    with pytest.raises(ExecutionRuntimeError, match="invalid-execution-event"):
        journal.append(
            "run-policy-binding",
            EXECUTION_POLICY_DECIDED,
            {
    ...
    with pytest.raises(ExecutionRuntimeError, match="invalid-execution-transition"):
        journal.append(
            "run-policy-binding",
            EXECUTION_TASK_STARTED,
            {
...
def test_journal_binds_plan_to_goal_draft_and_dependency_order_before_append(
    ...
    foreign = compile_plan(replace(goal, goal_id="goal-foreign"), draft)
...
def test_projection_rejects_malformed_checkpoint_fields_and_task_state(
    ...
    for mutate in mutations:
        ...
        mutate(candidate)
        with pytest.raises((ExecutionRuntimeError, ValueError)):
...
def test_projection_rejects_invalid_or_contradictory_public_terminal_events(
    ...
    def public_event(
        ...
        *,
        current=state,
        stream_id: str = "run:run-public-terminal",
    ...
    with pytest.raises(ExecutionRuntimeError, match="invalid-execution-transition"):
        projection.apply(state, public_event(RUN_SUCCEEDED))
    with pytest.raises(ExecutionRuntimeError, match="invalid-execution-event"):
        projection.apply(
            state,
            public_event(RUN_FAILED, stream_id="wrong-stream"),
        )
    with pytest.raises(ExecutionRuntimeError, match="invalid-execution-event"):
        projection.apply(
            state,
            public_event(RUN_FAILED, sequence=state.last_stream_sequence + 2),
        )

    failed = projection.apply(state, public_event(RUN_FAILED))
    ...
    with pytest.raises(ExecutionRuntimeError, match="event-after-execution-terminal"):
        projection.apply(
            failed,
            public_event(RUN_CANCELED, current=failed),
        )
    escalated = projection.apply(state, public_event(RUN_RECONCILIATION_REQUIRED))
    assert escalated is not None
    assert escalated.status is RunLifecycleStatus.ESCALATED
    canceled = projection.apply(state, public_event(RUN_CANCELED))
    assert canceled is not None
    assert canceled.status is RunLifecycleStatus.CANCELED

    terminal_without_execution = EventEnvelope.create(
        stream_id="run:public-only",
        stream_sequence=1,
    ...
    assert projection.apply(None, terminal_without_execution) is None
    unknown = EventEnvelope.create(
        stream_id="run:run-public-terminal",
...
def test_journal_and_coordinator_fail_closed_across_remaining_boundary_errors(
    ...
    with pytest.raises(ExecutionRuntimeError, match="goal-not-found"):
        empty.goal("run-empty-boundaries")
    with pytest.raises(ExecutionRuntimeError, match="plan-not-found"):
    ...
    with pytest.raises(ExecutionRuntimeError, match="invalid-execution-run-id"):
        empty.append("../bad-run", EXECUTION_GOAL_ADMITTED, {}, actor="daemon:test")
    ...
    with pytest.raises(ExecutionRuntimeError, match="idempotency-conflict"):
        admitted.append(
            "run-admitted-boundaries",
            EXECUTION_GOAL_ADMITTED,
            admitted_payload,
    ...
    admitted_coordinator = ExecutionCoordinator(
        admitted,
        _Provider(_draft(tasks=(_task("implement", allowed_paths=("src",)),))),
        _Executor((_success(),)),
        ExecutionPolicyKernel(),
    ...
    with pytest.raises(ExecutionRuntimeError, match="planning-not-ambiguous"):
    ...
    running_coordinator = ExecutionCoordinator(
        running,
        _Provider(_draft(tasks=(_task("implement", allowed_paths=("src",)),))),
        _Executor((_success(),)),
        ExecutionPolicyKernel(),
    ...
    running_decision = ExecutionPolicyKernel().authorize(
    ...
    with pytest.raises(ExecutionRuntimeError, match="replan-not-required"):
        running_coordinator.exhaust_replan_budget("run-running-boundaries", actor="daemon:worker")
    with pytest.raises(ExecutionRuntimeError, match="run-binding-mismatch"):
    ...
    exhausted = ExecutionCoordinator(
    ...
    overdrawn = ExecutionCoordinator(
    ...
    first = EventEnvelope.create(
        stream_id="run:run-public-prefix",
        stream_sequence=1,
    ...
    public_journal = EventBackedExecutionRunJournal(
        public_events,
        CheckpointStore(public_database),
    )
    with pytest.raises(ExecutionRuntimeError, match="public-run-already-active"):
    ...
    corrupt_journal = EventBackedExecutionRunJournal(
        corrupt_events,
...
def test_journal_surfaces_concurrent_append_without_persisting_candidate(
    ...
    def race(event: EventEnvelope, *, expected_sequence: int) -> EventEnvelope:
        competing = EventEnvelope.create(
            stream_id="run:run-concurrent",
            stream_sequence=2,
            event_type=EXECUTION_PLAN_DRAFT_RECEIVED,
            actor="other:planner",
    ...
    with pytest.raises(ExecutionRuntimeError, match="concurrency-conflict"):
        journal.append(
            "run-concurrent",
            EXECUTION_PLAN_DRAFT_RECEIVED,
            {
    ...
    assert tuple(item.actor for item in events.read_stream("run:run-concurrent")) == (
        "daemon:planner",
        "other:planner",
    )
...
def test_replan_creates_one_immutable_successor_then_executes(tmp_path: Path) -> None:
    ...
    assert first_plan.plan_revision == 1
...
def test_second_replan_request_exhausts_budget_without_another_provider_call(
    ...
    assert journal.plan(request.run_id).plan_revision == 2
...
def test_dependency_heads_cover_diamond_redundant_and_divergent_edges(tmp_path: Path) -> None:
    ...
    divergent_coordinator = ExecutionCoordinator(
...
def test_event_journal_exports_run_plan_task_and_attempt_correlations(tmp_path: Path) -> None:
    ...
    started = next(
        item for item in records if item.attributes["event.type"] == EXECUTION_TASK_STARTED
    )
    ...
    terminal = next(
        item for item in records if item.attributes["event.type"] == EXECUTION_RUN_TERMINATED
    )
...
def test_trace_observer_prefers_payload_run_and_omits_non_typed_optional_metadata() -> None:
    ...
    event = EventEnvelope.create(
        stream_id="run:payload-run",
        stream_sequence=1,
        event_type=EXECUTION_TASK_STARTED,
        actor="daemon:test",
        source=EXECUTION_EVENT_SOURCE,
        payload={
            "run_id": "payload-run",
...
def test_execution_contracts_reject_malformed_identity_scope_and_evidence() -> None:
    goal = _goal()
    check = VerificationCheck("check-valid", ("pytest", "-q"))
    task = TaskSpec("task-valid", "Do bounded work.", (), ("src",), (check,), 3)
    plan = compile_plan(goal, _draft(tasks=(_task("implement", allowed_paths=("src",)),)))
    result = _Provider(_draft(tasks=(_task("implement", allowed_paths=("src",)),))).propose_plan(
        _planning_request("run-contracts")
    )

    invalid_goals = (
        lambda: replace(goal, schema_version="blackcell.goal/v1"),
    ...
    invalid_checks = (
        ("empty argv", lambda: VerificationCheck("check", ())),
        ("unsafe executable", lambda: VerificationCheck("check", ("../pytest",))),
        ("nul token", lambda: VerificationCheck("check", ("pytest", "bad\x00token"))),
        ("exit range", lambda: VerificationCheck("check", ("pytest",), 256)),
    )
    ...
    invalid_plans = (
        lambda: replace(plan, schema_version="blackcell.plan/v1"),
        lambda: replace(plan, plan_revision=0),
        lambda: replace(plan, plan_revision=2, supersedes_plan_id=None),
        lambda: replace(plan, plan_revision=2, supersedes_plan_id=plan.plan_id),
    ...
    successful = _success()
    invalid_evidence = (
        lambda: replace(successful, schema_version="blackcell.attempt-evidence/v0"),
...
def test_runtime_fails_closed_on_denial_executor_error_and_ambiguous_recovery(
    ...
    error_coordinator = ExecutionCoordinator(
        error_journal,
        _Provider(_draft(tasks=(_task("implement", allowed_paths=("src",)),))),
        _RaisingExecutor(),
        ExecutionPolicyKernel(),
    ...
    recovery_coordinator = ExecutionCoordinator(
        recovery_journal,
        _Provider(_draft(tasks=(_task("implement", allowed_paths=("src",)),))),
        _Executor((_success(),)),
        ExecutionPolicyKernel(),
    ...
    recovery_decision = ExecutionPolicyKernel().authorize(
    ...
    with pytest.raises(ExecutionRuntimeError, match="binding-mismatch"):
    ...
    with pytest.raises(ExecutionRuntimeError, match="checkpoint"):
...
class _DenyPolicy(ExecutionPolicyKernel):
    def authorize(
        self,
        goal: GoalSpec,
        plan: Plan,
...
def _journal(tmp_path: Path) -> EventBackedExecutionRunJournal:
    path = tmp_path / "kernel.sqlite3"
    return EventBackedExecutionRunJournal(EventStore(path), CheckpointStore(path))


def _named_journal(tmp_path: Path, name: str) -> EventBackedExecutionRunJournal:
...
def _goal(*, allowed_paths: tuple[str, ...] = ("src",)) -> GoalSpec:
    return GoalSpec(
        ...
        verification_checks=(VerificationCheck("check", ("pytest", "-q")),),
    )


def _draft(*, tasks: tuple[dict[str, object], ...]) -> dict[str, object]:
    return {"schema_version": EXECUTION_PLAN_DRAFT_SCHEMA, "tasks": list(tasks)}
