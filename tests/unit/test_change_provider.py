...
def test_gateway_provider_uses_code_capability_closed_schema_and_no_tools() -> None:
    ...
    assert result.profile_id == "execution"
    ...
    assert request.input["schema_version"] == "change-context/v1"
...
def test_gateway_provider_rejects_evidence_mismatch_and_invalid_output() -> None:
    context = _context()
    mismatch = GatewayChangeProvider(RecordingGateway(_output("sha256:" + "9" * 64)))
    with pytest.raises(ChangeProviderError) as mismatched:
        mismatch.propose(_call(context))
    assert mismatched.value.code is ChangeProviderFailureCode.EVIDENCE_MISMATCH
    assert context.objective not in str(mismatched.value)

    malformed = _output(context.digest)
    malformed["unexpected"] = True
    with pytest.raises(ChangeProviderError) as invalid:
        GatewayChangeProvider(RecordingGateway(malformed)).propose(_call(context))
    assert invalid.value.code is ChangeProviderFailureCode.INVALID_PROPOSAL
...
def _call(context: ChangeContext) -> ChangeProviderCall:
    return ChangeProviderCall(
