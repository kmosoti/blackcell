...
from blackcell.bootstrap.execution_worker import ExecutionWorker, ExecutionWorkerPolicy
...
from blackcell.orchestration.execution_artifacts import OUTCOME_MEDIA_TYPE
...
from tests.unit.test_execution_worker import (
    RecordingAcceptance,
    ReplacingProvider,
    _git,
    _git_text,
...
def test_completed_worker_run_replays_complete_artifact_chain_live_free(
    ...
    assert first == second
    assert first.schema_version == "replay/v2"
...
def test_replay_reports_missing_linked_artifact(tmp_path: Path) -> None:
    ...
    assert replay.findings[0].code == "replay-artifact-missing"
...
def test_replay_reports_changed_artifact_bytes(tmp_path: Path) -> None:
    ...
    assert replay.artifact_integrity == "failed"
    assert replay.findings[0].code == "replay-artifact-integrity-failed"
    assert replay.findings[0].role == "context"
    assert replay.findings[0].artifact_digest == context_digest
...
def test_replay_reports_unavailable_store_and_absent_terminal_outcome_as_inconclusive(
    ...
    assert unavailable.findings[0].code == "replay-artifact-store-unavailable"
    assert absent.status is ExecutionArtifactReplayStatus.INCONCLUSIVE
    assert absent.findings[0].code is ReplayFindingCode.OUTCOME_REFERENCE_ABSENT
...
def test_replay_rejects_noncanonical_or_source_unbound_outcome(tmp_path: Path) -> None:
    ...
    noncanonical = verify_run_artifacts(
        artifacts,
        run_id="run-1",
    ...
    unbound = verify_run_artifacts(
        artifacts,
        run_id="run-1",
        nodes=(replace(original, result_digest=unbound_ref.digest),),
    )

    assert noncanonical.status is ExecutionArtifactReplayStatus.FAILED
    assert noncanonical.findings[0].code is ReplayFindingCode.ARTIFACT_NONCANONICAL
    assert unbound.status is ExecutionArtifactReplayStatus.FAILED
    assert unbound.findings[0].code is ReplayFindingCode.ARTIFACT_BINDING_MISMATCH
...
def _absent_failed_expectation() -> ReplayNodeExpectation:
    return ReplayNodeExpectation(
        node_id="failed",
        ...
        base_commit="c" * 40,
        head_commit=None,
        failure_code="execution-worker-artifact-failed",
        result_digest=None,
