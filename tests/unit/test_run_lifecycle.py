...
def test_success_records_retained_head_and_dependent_writer_inherits_it(
    ...
    service, events, repository, isolation, base_commit = _runtime(tmp_path)
    budget = NodeBudget(1_000, 1_000, 60, 1_000, 2)
    nodes = (
        PlanNode(
            node_id="write-first",
            ...
            allowed_paths=("src",),
            checks=(AcceptanceCheck("first-check", ("python", "-m", "compileall", "src")),),
        ),
        PlanNode(
            ...
            effects=("repository-read", "repository-write", "process"),
            allowed_paths=("src",),
            checks=(AcceptanceCheck("second-check", ("python", "-m", "compileall", "src")),),
        ),
    )
...
def test_startup_reconciliation_requeues_unchanged_and_retains_changed_work(
    ...
    for stale_spec in (missing.spec, changed.spec):
        ...
        assert stale.value.code is RuntimeApiFailureCode.CONFLICT
    assert _run_event_types(events, "run-missing")[-1] == "node.requeued"
...
def test_provider_dispatch_marker_is_durable_fenced_and_strict(tmp_path: Path) -> None:
    ...
    request_id = provider_request_id(prepared.spec.lease.digest)
    ...
    with pytest.raises(RuntimeApiError) as wrong_request:
        service.record_provider_dispatch(
            prepared.spec,
            provider_request_id="change-wrong",
            context_digest=_CONTEXT_DIGEST,
            context_artifact_digest=_CONTEXT_DIGEST,
            principal_id="worker-1",
        )
    ...
    stream = events.read_stream("run:run-1")
    dispatch = stream[-1]
    ...
    state = fold_run_lifecycle("run-1", {"change": ()}, stream)
    ...
    with pytest.raises(LifecycleError):
        fold_run_lifecycle("run-1", {"change": ()}, (*stream[:2], unprepared_dispatch))
    ...
    with pytest.raises(RuntimeApiError) as stale:
        service.record_provider_dispatch(
            stale_spec,
            provider_request_id=provider_request_id(stale_spec.lease.digest),
            context_digest=_CONTEXT_DIGEST,
            context_artifact_digest=_CONTEXT_DIGEST,
            principal_id="worker-1",
        )
    ...
    readonly_budget = NodeBudget(0, 0, 60, 0, 0)
    _submit(
        readonly,
        readonly_repository,
        readonly_base,
        ...
        nodes=(
            PlanNode(
                ...
                allowed_paths=(),
                checks=(AcceptanceCheck("compile", ("python", "-m", "compileall", "src")),),
            ),
        ),
    )
    ...
    with pytest.raises(RuntimeApiError) as readonly_dispatch:
        readonly.record_provider_dispatch(
            ...
            provider_request_id=provider_request_id(readonly_prepared.spec.lease.digest),
            context_digest=_CONTEXT_DIGEST,
            context_artifact_digest=_CONTEXT_DIGEST,
...
def test_startup_reconciliation_never_requeues_ambiguous_provider_dispatch(
    ...
    for prepared in prepared_by_run.values():
        service.record_provider_dispatch(
            prepared.spec,
            provider_request_id=provider_request_id(prepared.spec.lease.digest),
            context_digest=_CONTEXT_DIGEST,
            context_artifact_digest=_CONTEXT_DIGEST,
    ...
    for run_id in ("run-missing", "run-unchanged", "run-changed"):
        ...
        assert "node.requeued" not in _run_event_types(events, run_id)
        reconciliation = next(
            event
            for event in events.read_stream(f"run:{run_id}")
            if event.event_type == "node.reconciliation-required"
        )
        assert reconciliation.payload["failure_code"] == "provider-dispatch-ambiguous"
    assert not by_id["run-missing"].retained_worktree
    assert by_id["run-unchanged"].retained_worktree
    assert by_id["run-changed"].retained_worktree
    assert changed_target.read_text() == "VALUE = 9\n"
    assert by_id["run-canceled"].status == "canceled"
    assert "node.requeued" not in _run_event_types(events, "run-canceled")
...
def _assert_successful_worktree_cleanup_recovery(
    ...
    *,
    ...
    assert _run_event_types(events, "run-1")[-1] == ("node.worktree-cleanup-requested")
...
def test_successful_worktree_cleanup_failure_is_durable_and_not_retried(
    ...
    failed = events.read_stream("run:run-1")[-1]
    assert failed.event_type == "node.worktree-cleanup-failed"
    ...
    event_count = len(events.read_stream("run:run-1"))
    ...
    assert len(events.read_stream("run:run-1")) == event_count
    assert dirty.read_text() == "LATE = True\n"


def _runtime(
    tmp_path: Path,
) -> tuple[RuntimeService, EventStore, Path, Path, str]:
    ...
    *,
    ...
    service.accept_intent(
        IntentRequest(
            ...
            intent_id="intent-1",
            project_id="project-1",
            objective="Apply one bounded execution change.",
...
def _run(run_id: str) -> RunRequest:
    return RunRequest(
...
def _run_event_types(events: EventStore, run_id: str) -> tuple[str, ...]:
    return tuple(event.event_type for event in events.read_stream(f"run:{run_id}"))
