...
import blackcell.orchestration.replay as replay
...
from blackcell.orchestration.review import ReviewContext
from tests.unit.test_execution_worker import RecordingAcceptance
...
def test_review_evidence_builds_exact_acceptance_and_all_verified_excerpts(
    ...
    assert context.acceptance.objective == "Apply and verify one bounded execution change."
    ...
    assert by_kind["source-before"].operation is TextOperation.REPLACE
    assert by_kind["source-before"].excerpt == "VALUE = 1\n"
    assert by_kind["source-after"].path == "src/value.py"
    assert by_kind["source-after"].operation is TextOperation.REPLACE
    assert by_kind["source-after"].excerpt == "VALUE = 2\n"
    assert by_kind["effect"].operation is TextOperation.REPLACE
...
def test_review_evidence_rejects_tamper_definition_drift_and_non_success(
    ...
    with pytest.raises(ReviewEvidenceError) as drift:
        build_review_context_from_artifacts(
            artifacts,
            run_id="run-1",
    ...
    assert drift.value.code is ReviewEvidenceFailureCode.DEFINITION_MISMATCH

    with pytest.raises(ReviewEvidenceError) as non_success:
        _build(
            artifacts,
            state_digest,
            replace(expectation, status="failed", failure_code="forced-failure"),
        )
    assert non_success.value.code is ReviewEvidenceFailureCode.ARTIFACTS_NOT_VERIFIED

    context_digest = _linked_digest(outcome, "context_artifact")
    artifacts.path_for(context_digest).write_bytes(b"tampered")
    with pytest.raises(ReviewEvidenceError) as tampered:
        _build(artifacts, state_digest, expectation)
    assert tampered.value.code is ReviewEvidenceFailureCode.ARTIFACTS_NOT_VERIFIED
...
def test_review_evidence_truncates_oversized_artifacts_and_enforces_aggregate_budget(
    ...
    def run_with_large_binary_stdout(
        ...
        *,
        ...
        return replace(result, stdout=AcceptanceStream(b"\xff" * (64 * 1024)))
    ...
    assert stdout.artifact_digest == AcceptanceStream(b"\xff" * (64 * 1024)).digest

    with monkeypatch.context() as patch:
        patch.setattr(replay, "_MAX_REVIEW_EVIDENCE_BYTES", 8 * 128)
    ...
    monkeypatch.setattr(replay, "_MAX_REVIEW_EVIDENCE_BYTES", 1)
    with pytest.raises(ReviewEvidenceError) as aggregate:
        _build(clean_artifacts, clean_state_digest, clean_expectation)
    assert aggregate.value.code is ReviewEvidenceFailureCode.EVIDENCE_BUDGET_EXCEEDED
...
def test_review_evidence_replays_two_checks_at_full_stream_capacity(
    ...
    def run_with_full_streams(
        ...
        *,
        ...
        stdout = AcceptanceStream(bytes((0xFC + offset,)) * MAX_ACCEPTANCE_STREAM_BYTES)
        stderr = AcceptanceStream(bytes((0xFD + offset,)) * MAX_ACCEPTANCE_STREAM_BYTES)
    ...
    runtime, _, artifacts, _, _, outcome, _, _ = _completed_writer(
        ...
        stream_limit_bytes=MAX_ACCEPTANCE_STREAM_BYTES,
...
def _build(
    ...
    state_digest: str,
    expectation: replay.ReplayNodeExpectation,
) -> ReviewContext:
