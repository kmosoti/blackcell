...
from blackcell.interfaces.tui.controller import TuiProjection
...
class _ShellController:
    def __init__(
        ...
        *,
        ...
        self.cancel_request: CancelRunRequest | None = None
...
def test_tui_app_projects_events_and_run_operations_without_blocking(
    tmp_path: Path,
) -> None:
    ...
    assert controller.cancel_request.schema_version == "execution-cancel-run-request/v1"


def test_tui_app_submits_bounded_workflow_and_projects_replay_evidence(
    tmp_path: Path,
) -> None:
    run, replay, canceled, events = _runtime_records(tmp_path)
    ...
    app = TuiApp(lambda: controller, event_refresh_seconds=None)
    ...
    async def exercise() -> None:
        ...
        for key, operation in zip("1234", ("project", "intent", "plan", "run"), strict=True):
            ...
            await app.wait_idle()
            assert app.view.message == f"tui-workflow-{operation}-complete"
        ...
        assert "replay-artifact-missing" in output
        assert "Review findings: unavailable in replay/v2" in output
...
def test_tui_app_rejects_unsafe_workflow_files_content_free(tmp_path: Path) -> None:
    run, replay, canceled, events = _runtime_records(tmp_path)
    controller = _ShellController(run=run, replay=replay, canceled=canceled, events=events)
    ...
    async def exercise() -> None:
        ...
        for operation_key, path in cases:
            ...
            app.handle_key(_Key("enter"))
            await app.wait_idle()
            assert app.view.message == "tui-invalid-workflow-request"
...
def test_tui_app_serializes_refresh_and_keeps_input_responsive(tmp_path: Path) -> None:
    run, replay, canceled, events = _runtime_records(tmp_path)
    gate = asyncio.Event()
    controller = _ShellController(
        run=run,
    ...
    async def exercise() -> None:
        ...
        gate.set()
        await app.wait_idle()
        assert app.view.message == "tui-operation-failed"
    ...
    with pytest.raises(ValueError, match="event_refresh_seconds"):
        TuiApp(lambda: controller, event_refresh_seconds=0.1)
    with pytest.raises(ValueError, match="frames_per_second"):
        TuiApp(lambda: controller, frames_per_second=0)


def test_tui_app_runs_and_restores_injected_async_terminal(tmp_path: Path) -> None:
    run, replay, canceled, events = _runtime_records(tmp_path)
    controller = _ShellController(run=run, replay=replay, canceled=canceled, events=events)
...
def test_tui_projects_generated_dag_attempt_limits_and_unknown_usage(
    tmp_path: Path,
) -> None:
    ...
    discovered = RunQueryItem(
        queued_cursor=1,
        run=run,
        nodes=nodes,
        usage=usage,
    )
    state = TuiProjection(runs=(discovered,), run=run)
...
def _edit(app: TuiApp, key: str, value: str) -> None:
    assert app.handle_key(_Key(key))
    for character in value:
        assert app.handle_key(_Key(character))


def _workflow_requests(replay: ReplayResponse) -> dict[str, StrictStruct]:
