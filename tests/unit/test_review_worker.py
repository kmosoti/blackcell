...
from tests.unit.test_replay import _completed_writer, _linked_digest
...
class RecordingReviewer:
    def __init__(self, *, context_mismatch: bool = False) -> None:
        ...
        self.calls: list[ReviewProviderCall] = []
    ...
    def review(self, call: ReviewProviderCall) -> ReviewProviderResult:
        ...
        proposal = ReviewProposal(
            ...
            epistemic_assessments=_assessments(call.context, ("finding-1",)),
        )
...
def test_review_worker_persists_context_dispatch_proposal_provider_and_admission(
    ...
    stream = events.read_stream(review_stream("run-1"))
    ...
    assert call.request_id == review_provider_request_id(state.lease.digest)
    ...
    reopened_execution = RuntimeService(
    ...
    reopened = _worker(
        reopened_execution,
        ReviewRuntimeService(EventStore(events.path)),
...
def test_review_worker_renews_after_preparation_and_preserves_completion_reserve(
    ...
    state = ReviewRuntimeService(events).inspect("run-1")
    assert state is not None
    assert state.lease.expires_at == NOW + timedelta(seconds=225)
    assert REVIEW_LEASE_RENEWED in tuple(
        event.event_type for event in events.read_stream(review_stream("run-1"))
    )
    with pytest.raises(ValueError, match="invalid review worker policy"):
        ReviewWorkerPolicy(
            worker_id="reviewer-1",
            budget=GatewayBudget(20_000, 2_000, 180_000, 10_000),
            lease_seconds=180,
...
def test_review_worker_renews_after_preparation_outlasts_the_original_lease(
    ...
    assert result.status == "review-succeeded"
    assert reviewer.calls[0].budget.max_latency_ms == 180_000
    event_types = tuple(event.event_type for event in events.read_stream(review_stream("run-1")))
...
def test_review_worker_records_stable_preparation_provider_and_admission_failures(
    ...
    assert preparation.failure_code == "review-artifacts-not-verified"
    preparation_state = ReviewRuntimeService(events).inspect("run-1")
    assert preparation_state is not None
    assert preparation_state.status is ReviewLifecycleStatus.FAILED
    ...
    assert provider.failure_code == "review-provider-failed"
    assert "sensitive" not in repr(provider)
    assert events.read_stream(review_stream("run-1"))[-1].event_type == REVIEW_FAILED
    ...
    assert admission.failure_code == "review-admission-rejected"
...
def test_review_worker_restart_does_not_repeat_completed_or_dispatched_review(
    ...
    assert dispatched.status is ReviewLifecycleStatus.PROVIDER_DISPATCHED
    ...
    report = ReviewRuntimeService(EventStore(events.path)).reconcile(
        principal_id="review-supervisor"
    )
    assert report.ambiguous_run_ids == ("run-1",)
    assert events.read_stream(review_stream("run-1"))[-1].event_type == (
        REVIEW_RECONCILIATION_REQUIRED
    )


def test_review_worker_ports_exclude_execution_and_worktree_authority() -> None:
    assert tuple(item.name for item in fields(ReviewWorker)) == (
        "execution",
        "scheduler",
        "artifacts",
        "reviewer",
...
def _worker(
    ...
    scheduler: ReviewRuntimeService,
