...
def test_review_lifecycle_folds_claim_dispatch_and_source_bound_success() -> None:
    ...
    dispatch = _event(
        lease,
        sequence=2,
        event_type=REVIEW_PROVIDER_DISPATCH_STARTED,
        causation_id=claim.event_id,
        payload={
            "run_id": lease.run_id,
            "review_id": lease.review_id,
            "lease_digest": lease.digest,
            "provider_request_id": review_provider_request_id(lease.digest),
    ...
    state = fold_review_lifecycle(lease.run_id, (claim, dispatch, success))
    payload = review_lifecycle_payload(state)
    ...
    assert state.provider_request_id == review_provider_request_id(lease.digest)
...
def test_review_lifecycle_rejects_stale_fences_unknown_fields_and_self_admission() -> None:
    ...
    with pytest.raises(ReviewLifecycleError):
        fold_review_lifecycle(stale.run_id, (_claim(stale),))

    wrong_cause = _claim(lease, causation_id="different-event")
    with pytest.raises(ReviewLifecycleError):
        fold_review_lifecycle(lease.run_id, (wrong_cause,))

    unknown = review_lease_payload(lease)
    claim_with_admission = EventEnvelope.create(
        stream_id=review_stream(lease.run_id),
        stream_sequence=1,
    ...
    with pytest.raises(ReviewLifecycleError):
        fold_review_lifecycle(lease.run_id, (claim_with_admission,))

    substituted = replace(_claim(lease), stream_id=f"run:{lease.run_id}")
    with pytest.raises(ReviewLifecycleError):
        fold_review_lifecycle(lease.run_id, (substituted,))
...
def test_review_lifecycle_renews_only_the_exact_claimed_lease_after_expiry() -> None:
    ...
    renewal = _event(
        renewed,
        sequence=2,
        event_type=REVIEW_LEASE_RENEWED,
        causation_id=claim.event_id,
    ...
    state = fold_review_lifecycle(lease.run_id, (claim, renewal))

    assert state.status is ReviewLifecycleStatus.CLAIMED
    ...
    invalid = _event(
        ...
        payload={
            ...
            "lease": review_lease_payload(changed_fence),
            "status": "claimed",
        },
    )
    with pytest.raises(ReviewLifecycleError):
        fold_review_lifecycle(lease.run_id, (claim, invalid))


def _lease() -> ReviewLease:
    run_id = "run-1"
    return ReviewLease(
        run_id=run_id,
        review_id=review_id(run_id, RUN_EVENT_DIGEST),
        attempt=1,
        fencing_token=1,
