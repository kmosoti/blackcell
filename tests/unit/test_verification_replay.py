...
from blackcell.bootstrap.verification_worker import VerificationSchedulerPort
...
from blackcell.orchestration.verification_replay import (
...
def test_verification_replay_validates_completed_report_and_restart(tmp_path: Path) -> None:
    ...
    first = replay_verification(source.events, artifacts, run_id="run-1")
    reopened_events = EventStore(source.events.path)
    second = replay_verification(
        reopened_events,
    ...
    assert first == second
    assert first.lifecycle_status is VerificationReplayLifecycle.COMPLETED
    assert first.verdict is VerificationStatus.PASS
    ...
    assert first.report_media_type == "application/vnd.blackcell.verification-report+json"
...
def test_verification_replay_distinguishes_not_started_incomplete_and_verifier_error(
    ...
    not_started = replay_verification(source.events, artifacts, run_id="run-1")
    ...
    claimed = replay_verification(source.events, artifacts, run_id="run-1")
    scheduler.reconcile(principal_id="verification-supervisor")
    requeued = replay_verification(source.events, artifacts, run_id="run-1")

    assert not_started.lifecycle_status is VerificationReplayLifecycle.NOT_STARTED
    assert not_started.artifact_integrity.value == "not-applicable"
    assert claimed.lifecycle_status is VerificationReplayLifecycle.CLAIMED
    assert claimed.artifact_integrity.value == "not-applicable"
    assert requeued.lifecycle_status is VerificationReplayLifecycle.REQUEUED
    ...
    verifier_error = replay_verification(source.events, artifacts, run_id="run-1")
    assert result.failure_code == "verifier-failed"
    assert verifier_error.lifecycle_status is VerificationReplayLifecycle.VERIFIER_ERROR
    assert verifier_error.verdict is None
    assert verifier_error.failure_code == "verifier-failed"
    ...
    persistence_error = replay_verification(source.events, artifacts, run_id="run-1")
    assert result.failure_code == "verification-persistence-failed"
    assert persistence_error.lifecycle_status is VerificationReplayLifecycle.VERIFIER_ERROR
    assert persistence_error.verdict is None
    assert persistence_error.failure_code == "verification-persistence-failed"
...
def test_verification_replay_rejects_tamper_metadata_and_source_binding_drift(
    ...
    tampered = replay_verification(source.events, artifacts, run_id="run-1")
    assert tampered.artifact_integrity.value == "failed"
    assert tampered.finding_code is VerificationReplayFindingCode.REPORT_INTEGRITY_FAILED
    ...
    metadata = replay_verification(
        source.events,
        MetadataMismatchReader(artifacts),
        run_id="run-1",
    )
    unavailable = replay_verification(source.events, None, run_id="run-1")
    state = scheduler.inspect("run-1")
    assert state is not None
    source_drift = replay_verification(
    ...
    assert metadata.finding_code is VerificationReplayFindingCode.REPORT_METADATA_MISMATCH
    assert unavailable.artifact_integrity.value == "inconclusive"
    assert unavailable.finding_code is VerificationReplayFindingCode.ARTIFACT_STORE_UNAVAILABLE
    assert source_drift.artifact_integrity.value == "failed"
    assert source_drift.finding_code is VerificationReplayFindingCode.SOURCE_BINDING_MISMATCH
