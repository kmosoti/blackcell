...
class DispatchObservingProvider(ReplacingProvider):
    ...
    def propose(self, call: ChangeProviderCall) -> ChangeProviderResult:
        dispatch = self.events.read_stream(f"run:{call.run_id}")[-1]
        assert dispatch.event_type == "node.provider-dispatch-started"
...
class FailingProvider:
    def propose(self, call: ChangeProviderCall) -> ChangeProviderResult:
        del call
        error = ChangeProviderError(ChangeProviderFailureCode.INVALID_GATEWAY_RESULT)
...
class RecordingAcceptance:
    def __init__(
        ...
        *,
        ...
        self.commands: list[AcceptanceCommand] = []
    ...
    def run(
        ...
        *,
        ...
        return AcceptanceResult(
            ...
            passed=passed,
            stdout=AcceptanceStream(f"{command.check_id}\n".encode()),
...
def test_worker_executes_writer_then_check_from_persisted_artifact_chain(
    ...
    worker = ExecutionWorker(
        ...
        worktrees=lifecycle,
        policy=ExecutionWorkerPolicy("worker-1"),
    )
...
def test_worker_lease_covers_worktrees_provider_and_each_acceptance_deadline(
    ...
    monkeypatch.setattr("blackcell.bootstrap.execution_worker.utc_now", lambda: claimed_at)
    lifecycle = GitWorktreeLifecycle()
    acceptance = RecordingAcceptance(lifecycle)
    worker = ExecutionWorker(
        runtime=runtime,
        ...
        acceptance=acceptance,
        worktrees=lifecycle,
        policy=ExecutionWorkerPolicy("worker-1", lease_grace_seconds=17),
    )
    ...
    claimed = next(
        event for event in events.read_stream("run:run-1") if event.event_type == "node.claimed"
    )
    assert MAX_EXECUTION_READ_BASE_WORKTREE_SECONDS == 48 * 120
    assert MAX_EXECUTION_WRITE_BASE_WORKTREE_SECONDS == 78 * 120
    assert MAX_EXECUTION_ACCEPTANCE_WORKTREE_SECONDS == 20 * 120
    assert datetime.fromisoformat(cast("str", claimed.payload["expires_at"])) == (
        claimed_at
        + timedelta(
            seconds=(
                MAX_EXECUTION_WRITE_BASE_WORKTREE_SECONDS
                + 2 * MAX_EXECUTION_ACCEPTANCE_WORKTREE_SECONDS
                + 30 * 3
                + 2 * 2
                + 17
            )
...
def test_worker_records_content_free_provider_failure_and_retains_checkout(
    ...
    assert result.failure_code == "invalid-change-gateway-result"
    ...
    failed = next(
        event for event in events.read_stream("run:run-1") if event.event_type == "node.failed"
    )
    ...
    serialized = repr(tuple(event.payload for event in events.read_stream("run:run-1")))
...
def test_worker_records_context_and_dispatch_before_provider_call(tmp_path: Path) -> None:
    ...
    dispatch = next(
        event
        for event in events.read_stream("run:run-1")
        if event.event_id == provider.dispatch_event_id
    )
    prepared = events.read_stream("run:run-1")[2]
...
def test_worker_persists_failed_check_streams_and_committed_recovery_head(
    ...
    assert result.status == "node-failed"
    assert result.failure_code == "execution-acceptance-check-failed"
    assert result.outcome_artifact_digest is not None
    outcome = cast("dict[str, object]", artifacts.get_json(result.outcome_artifact_digest))
    assert outcome["status"] == "failed"
...
def test_worker_acknowledges_cancellation_from_acceptance_callback(tmp_path: Path) -> None:
    ...
    canceled = next(
        event for event in events.read_stream("run:run-1") if event.event_type == "node.canceled"
    )
...
def test_worker_converts_success_race_to_durable_canceled_outcome(tmp_path: Path) -> None:
    ...
    _submit(runtime, repository, base_commit, writer_only=True)
    lifecycle = GitWorktreeLifecycle()
    racing_runtime = cast("ExecutionWorkerPort", CancelBeforeSuccessRuntime(runtime))
    worker = ExecutionWorker(
...
def _runtime(
    tmp_path: Path,
) -> tuple[RuntimeService, EventStore, ArtifactStore, Path, Path, str]:
    ...
    *,
    ...
    runtime.accept_intent(
        IntentRequest(
            ...
            intent_id="intent-1",
            project_id="project-1",
            objective="Apply and verify one bounded execution change.",
    ...
    check_budget = NodeBudget(0, 0, 30, 0, 0)
    writer = PlanNode(
        node_id="write",
        ...
        checks=tuple(
            AcceptanceCheck(check_id, ("python", "-m", "compileall", "src"))
    ...
    verify = PlanNode(
        ...
        allowed_paths=(),
        checks=(AcceptanceCheck("verify-check", ("python", "-m", "compileall", "src")),),
    )
    runtime.accept_plan(
        PlanRequest(
