from __future__ import annotations

import msgspec
import pytest

from blackcell.interfaces.http import (
    AcceptanceCheck,
    CancelRunRequest,
    NodeBudget,
    PlanNode,
    PlanRequest,
    ProjectRequest,
    RunQueryRequest,
    RuntimeEventResponse,
    WireContractError,
    decode_contract,
    plan_topological_order,
)
from blackcell.orchestration.acceptance import MAX_ACCEPTANCE_TIMEOUT_SECONDS

_DIGEST = "sha256:" + ("a" * 64)
_BASE_COMMIT = "b" * 40


def test_runtime_contracts_are_closed_and_bounded() -> None:
    request = decode_contract(
        msgspec.json.encode(
            {
                "schema_version": "project-request/v1",
                "project_id": "project-1",
                "root": "/tmp/project",
                "configuration_provider": "kernform",
                "configuration_version": "0.2.0",
                "configuration_digest": _DIGEST,
                "idempotency_key": "project-1",
            }
        ),
        ProjectRequest,
    )
    assert request.project_id == "project-1"

    for invalid in (
        {
            "schema_version": "project-request/v2",
            "project_id": "project-1",
            "root": "/tmp/project",
            "configuration_provider": "kernform",
            "configuration_version": "0.2.0",
            "configuration_digest": _DIGEST,
            "idempotency_key": "project-1",
        },
        {
            "schema_version": "project-request/v1",
            "project_id": "project-1",
            "root": "/tmp/project",
            "configuration_provider": "kernform",
            "configuration_version": "0.2.0",
            "configuration_digest": _DIGEST,
            "idempotency_key": "project-1",
            "unknown": True,
        },
        {
            "schema_version": "project-request/v1",
            "project_id": "project:invalid",
            "root": "/tmp/project",
            "configuration_provider": "kernform",
            "configuration_version": "0.2.0",
            "configuration_digest": "not-a-digest",
            "idempotency_key": "project-1",
        },
    ):
        with pytest.raises(WireContractError):
            decode_contract(msgspec.json.encode(invalid), ProjectRequest)

    plan = _valid_plan()
    assert plan_topological_order(plan.nodes) == ("inspect", "verify")


def test_plan_rejects_cycles_undeclared_effects_and_untestable_nodes() -> None:
    valid = msgspec.to_builtins(_valid_plan())
    assert isinstance(valid, dict)

    cyclic = {**valid, "nodes": [dict(node) for node in valid["nodes"]]}
    cyclic["nodes"][0]["depends_on"] = ["verify"]
    with pytest.raises(WireContractError):
        decode_contract(msgspec.json.encode(cyclic), PlanRequest)

    undeclared = {**valid, "nodes": [dict(node) for node in valid["nodes"]]}
    undeclared["nodes"][1]["effects"] = ["network"]
    with pytest.raises(WireContractError):
        decode_contract(msgspec.json.encode(undeclared), PlanRequest)

    untestable = {**valid, "nodes": [dict(node) for node in valid["nodes"]]}
    untestable["nodes"][0]["checks"] = []
    with pytest.raises(WireContractError):
        decode_contract(msgspec.json.encode(untestable), PlanRequest)

    unbounded_write = {**valid, "nodes": [dict(node) for node in valid["nodes"]]}
    unbounded_write["allowed_effects"] = ["repository-write", "process"]
    unbounded_write["nodes"][0]["effects"] = ["repository-write"]
    with pytest.raises(WireContractError):
        decode_contract(msgspec.json.encode(unbounded_write), PlanRequest)


def test_runtime_cancel_contract_is_closed_and_bounded() -> None:
    valid = {
        "schema_version": "execution-cancel-run-request/v1",
        "idempotency_key": "cancel-run-1",
    }
    decoded = decode_contract(msgspec.json.encode(valid), CancelRunRequest)
    assert decoded.idempotency_key == "cancel-run-1"

    for invalid in (
        {**valid, "schema_version": "execution-cancel-run-request/v2"},
        {**valid, "unexpected": True},
        {**valid, "idempotency_key": "invalid:key"},
    ):
        with pytest.raises(WireContractError):
            decode_contract(msgspec.json.encode(invalid), CancelRunRequest)


def test_run_query_contract_is_closed_canonical_and_bounded() -> None:
    request = decode_contract(
        msgspec.json.encode(
            {
                "schema_version": "run-query-request/v1",
                "statuses": ["succeeded", "queued"],
                "project_ids": ["project-2", "project-1"],
                "run_ids": ["run-2", "run-1"],
                "limit": 25,
            }
        ),
        RunQueryRequest,
    )

    assert request.statuses == ("queued", "succeeded")
    assert request.project_ids == ("project-1", "project-2")
    assert request.run_ids == ("run-1", "run-2")
    assert request.after_cursor == 0

    valid = msgspec.to_builtins(request)
    assert isinstance(valid, dict)
    for invalid in (
        {**valid, "schema_version": "run-query-request/v2"},
        {**valid, "unexpected": True},
        {**valid, "statuses": ["queued", "queued"]},
        {**valid, "statuses": ["unknown"]},
        {**valid, "project_ids": ["invalid:project"]},
        {**valid, "after_cursor": True},
        {**valid, "limit": 0},
        {**valid, "limit": 101},
    ):
        with pytest.raises(WireContractError):
            decode_contract(msgspec.json.encode(invalid), RunQueryRequest)


def test_runtime_event_contract_accepts_provider_dispatch_marker() -> None:
    event = decode_contract(
        msgspec.json.encode(
            {
                "schema_version": "event/v1",
                "event_id": "event-1",
                "cursor": 1,
                "stream_id": "run:run-1",
                "stream_sequence": 4,
                "event_type": "node.provider-dispatch-started",
                "event_schema_version": 1,
                "recorded_at": "2026-07-22T18:00:00+00:00",
                "correlation_id": "correlation-1",
                "causation_id": "prepared-event",
                "actor": "worker-1",
                "payload_digest": _DIGEST,
                "payload": {
                    "provider_request_id": "change-request",
                    "context_digest": _DIGEST,
                },
            }
        ),
        RuntimeEventResponse,
    )
    assert event.event_type == "node.provider-dispatch-started"


def test_runtime_event_contract_accepts_worktree_cleanup_events() -> None:
    for sequence, event_type in enumerate(
        (
            "node.worktree-cleanup-requested",
            "node.worktree-cleaned",
            "node.worktree-cleanup-failed",
        ),
        start=1,
    ):
        event = decode_contract(
            msgspec.json.encode(
                {
                    "schema_version": "event/v1",
                    "event_id": f"event-{sequence}",
                    "cursor": sequence,
                    "stream_id": "run:run-1",
                    "stream_sequence": sequence,
                    "event_type": event_type,
                    "event_schema_version": 1,
                    "recorded_at": "2026-07-22T18:00:00+00:00",
                    "correlation_id": "correlation-1",
                    "causation_id": "success-event",
                    "actor": "retention-worker",
                    "payload_digest": _DIGEST,
                    "payload": {"status": event_type.removeprefix("node.")},
                }
            ),
            RuntimeEventResponse,
        )
        assert event.event_type == event_type


def test_runtime_event_contract_accepts_review_and_verification_events() -> None:
    event_types = (
        "review.claimed",
        "review.lease-renewed",
        "review.provider-dispatch-started",
        "review.succeeded",
        "review.failed",
        "review.requeued",
        "review.reconciliation-required",
        "verification.claimed",
        "verification.completed",
        "verification.failed",
        "verification.requeued",
    )
    for sequence, event_type in enumerate(event_types, start=1):
        event = decode_contract(
            msgspec.json.encode(
                {
                    "schema_version": "event/v1",
                    "event_id": f"event-{sequence}",
                    "cursor": sequence,
                    "stream_id": "review:run-1",
                    "stream_sequence": sequence,
                    "event_type": event_type,
                    "event_schema_version": 1,
                    "recorded_at": "2026-07-22T18:00:00+00:00",
                    "correlation_id": "correlation-1",
                    "causation_id": "source-event",
                    "actor": "independent-worker",
                    "payload_digest": _DIGEST,
                    "payload": {"status": "bounded"},
                }
            ),
            RuntimeEventResponse,
        )
        assert event.event_type == event_type


def test_plan_requires_positive_model_budgets_only_for_repository_writers() -> None:
    check = AcceptanceCheck("check-1", ("python", "--version"))
    acceptance_only = PlanNode(
        node_id="accept",
        objective="Run acceptance without requesting a model change.",
        depends_on=(),
        budget=NodeBudget(0, 0, 30, 0, 0),
        effects=("repository-read", "process"),
        allowed_paths=(),
        checks=(check,),
    )

    assert acceptance_only.budget.max_input_tokens == 0
    assert acceptance_only.budget.max_output_tokens == 0
    for budget in (
        NodeBudget(0, 1_000, 30, 0, 1),
        NodeBudget(1_000, 0, 30, 0, 1),
    ):
        with pytest.raises(WireContractError):
            PlanNode(
                node_id="write",
                objective="Request a bounded repository change.",
                depends_on=(),
                budget=budget,
                effects=("repository-read", "repository-write", "process"),
                allowed_paths=("src/example.py",),
                checks=(check,),
            )


def test_plan_requires_explicit_check_effects_and_serial_writers() -> None:
    valid = msgspec.to_builtins(_valid_plan())
    assert isinstance(valid, dict)
    missing_process = {**valid, "nodes": [dict(node) for node in valid["nodes"]]}
    missing_process["nodes"][0]["effects"] = ["repository-read"]
    with pytest.raises(WireContractError):
        decode_contract(msgspec.json.encode(missing_process), PlanRequest)

    writer_budget = NodeBudget(1_000, 1_000, 30, 0, 1)
    writer = PlanNode(
        node_id="write-a",
        objective="Apply the first bounded change.",
        depends_on=(),
        budget=writer_budget,
        effects=("repository-read", "repository-write", "process"),
        allowed_paths=("src/a.py",),
        checks=(AcceptanceCheck("write-a-check", ("python", "-m", "compileall", "src")),),
    )
    parallel = PlanNode(
        node_id="write-b",
        objective="Apply the second bounded change.",
        depends_on=(),
        budget=writer_budget,
        effects=("repository-read", "repository-write", "process"),
        allowed_paths=("src/b.py",),
        checks=(AcceptanceCheck("write-b-check", ("python", "-m", "compileall", "src")),),
    )
    with pytest.raises(WireContractError):
        PlanRequest(
            schema_version="plan-request/v1",
            plan_id="parallel-writers",
            project_id="project-1",
            intent_id="intent-1",
            base_commit=_BASE_COMMIT,
            allowed_effects=("repository-read", "repository-write", "process"),
            nodes=(writer, parallel),
            idempotency_key="parallel-writers",
        )

    serial = msgspec.structs.replace(parallel, depends_on=(writer.node_id,))
    accepted = PlanRequest(
        schema_version="plan-request/v1",
        plan_id="serial-writers",
        project_id="project-1",
        intent_id="intent-1",
        base_commit=_BASE_COMMIT,
        allowed_effects=("repository-read", "repository-write", "process"),
        nodes=(writer, serial),
        idempotency_key="serial-writers",
    )
    assert plan_topological_order(accepted.nodes) == ("write-a", "write-b")


def test_node_budget_timeout_matches_executable_acceptance_limit() -> None:
    accepted = NodeBudget(1_000, 1_000, MAX_ACCEPTANCE_TIMEOUT_SECONDS, 0, 0)

    assert accepted.timeout_seconds == 600
    with pytest.raises(WireContractError):
        NodeBudget(1_000, 1_000, MAX_ACCEPTANCE_TIMEOUT_SECONDS + 1, 0, 0)


def test_runtime_check_identity_and_alias_match_executable_acceptance_contract() -> None:
    boundary = AcceptanceCheck("check-1", ("a" * 64, "--version"))

    assert boundary.argv[0] == "a" * 64
    for check_id in ("-check", "a" * 121):
        with pytest.raises(WireContractError):
            AcceptanceCheck(check_id, ("python", "--version"))
    for executable in ("tool/name", "a" * 65):
        with pytest.raises(WireContractError):
            AcceptanceCheck("check-1", (executable, "--version"))


def test_plan_paths_match_the_downstream_git_metadata_prohibition() -> None:
    budget = NodeBudget(1_000, 1_000, 30, 0, 1)

    for repository_path in (".git", "src/.git", "src/.git/config"):
        with pytest.raises(WireContractError):
            PlanNode(
                node_id="write",
                objective="Apply one bounded change.",
                depends_on=(),
                budget=budget,
                effects=("repository-read", "repository-write", "process"),
                allowed_paths=(repository_path,),
                checks=(AcceptanceCheck("write-check", ("python", "--version")),),
            )

    accepted = PlanNode(
        node_id="write",
        objective="Apply one bounded change.",
        depends_on=(),
        budget=budget,
        effects=("repository-read", "repository-write", "process"),
        allowed_paths=("src/.github/config.yml",),
        checks=(AcceptanceCheck("write-check", ("python", "--version")),),
    )
    assert accepted.allowed_paths == ("src/.github/config.yml",)


def _valid_plan() -> PlanRequest:
    budget = NodeBudget(
        max_input_tokens=1_000,
        max_output_tokens=1_000,
        timeout_seconds=30,
        max_cost_microusd=0,
        max_changed_files=0,
    )
    return PlanRequest(
        schema_version="plan-request/v1",
        plan_id="plan-1",
        project_id="project-1",
        intent_id="intent-1",
        base_commit=_BASE_COMMIT,
        allowed_effects=("repository-read", "process"),
        nodes=(
            PlanNode(
                node_id="inspect",
                objective="Inspect the bounded project surface.",
                depends_on=(),
                budget=budget,
                effects=("repository-read", "process"),
                allowed_paths=(),
                checks=(
                    AcceptanceCheck(
                        check_id="inspect-pass",
                        argv=("python", "-m", "compileall", "src"),
                    ),
                ),
            ),
            PlanNode(
                node_id="verify",
                objective="Verify the declared outcome.",
                depends_on=("inspect",),
                budget=budget,
                effects=("repository-read", "process"),
                allowed_paths=(),
                checks=(
                    AcceptanceCheck(
                        check_id="verify-pass",
                        argv=("pytest", "tests/unit/test_example.py", "-q"),
                    ),
                ),
            ),
        ),
        idempotency_key="plan-1",
    )
