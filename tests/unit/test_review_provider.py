...
from tests.unit.test_review_contracts import review_context, review_output
...
def test_gateway_reviewer_uses_review_capability_closed_schema_and_no_tools() -> None:
    ...
    assert json_digest(request.input) == json_digest(review_context_payload(context))
...
def test_gateway_reviewer_rejects_wrong_capability_identity_and_malformed_output() -> None:
    ...
    wrong_capability = GatewayReviewer(
        RecordingGateway(review_output(context), capability=ModelCapability.CODE)
    )
    with pytest.raises(ReviewProviderError) as capability_error:
        wrong_capability.review(call)
    assert capability_error.value.code is ReviewProviderFailureCode.INVALID_GATEWAY_RESULT

    identity_mismatch = GatewayReviewer(
        RecordingGateway(review_output(context), response_profile_id="different-profile")
    )
    with pytest.raises(ReviewProviderError) as identity_error:
        identity_mismatch.review(call)
    assert identity_error.value.code is ReviewProviderFailureCode.INVALID_GATEWAY_RESULT

    malformed = review_output(context)
    malformed["admitted"] = True
    with pytest.raises(ReviewProviderError) as proposal_error:
        GatewayReviewer(RecordingGateway(malformed)).review(call)
    assert proposal_error.value.code is ReviewProviderFailureCode.INVALID_PROPOSAL
...
def test_gateway_reviewer_rejects_context_mismatch_before_host_admission() -> None:
    ...
    with pytest.raises(ReviewProviderError) as error:
        GatewayReviewer(RecordingGateway(mismatch)).review(_call(context))

    assert error.value.code is ReviewProviderFailureCode.CONTEXT_MISMATCH
    assert context.acceptance.objective not in str(error.value)


def _call(context: ReviewContext) -> ReviewProviderCall:
    return ReviewProviderCall(
