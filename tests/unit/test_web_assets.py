...
def test_web_assets_are_bounded_packaged_and_closed() -> None:
...
def test_web_routes_emit_accessible_shell_and_restrictive_security_headers(
    tmp_path: Path,
) -> None:
    assets = load_web_assets()
    expected = {
...
def test_browser_client_keeps_credentials_in_memory_and_uses_only_runtime_contracts() -> None:
    assets = load_web_assets()
...
def test_browser_response_ceiling_matches_the_http_service_contract() -> None:
    javascript = load_web_assets().javascript.decode("utf-8")
...
def test_browser_text_validation_matches_the_service_multiline_contract() -> None:
    ...
    plan_node = PlanNode(
        ...
        depends_on=(),
        budget=NodeBudget(0, 0, 1, 0, 0),
        effects=("repository-read", "process"),
        allowed_paths=(),
        checks=(AcceptanceCheck("inspect-check", ("python",)),),
    )
...
def test_web_workflow_surface_is_bounded_closed_and_accessible() -> None:
    assets = load_web_assets()
    html = assets.html.decode("utf-8")
    javascript = assets.javascript.decode("utf-8")
    css = assets.css.decode("utf-8")
    ...
    for required in (
        ...
        'this._post("/api/v1/projects"',
        'this._post("/api/v1/intents"',
        'this._post("/api/v1/plans"',
        'this._post("/api/v1/runs"',
