...
def test_collector_returns_only_bounded_regular_utf8_files(tmp_path: Path) -> None:
    ...
    context = ExecutionEvidenceCollector().collect(
        spec,
...
def test_collector_rejects_symlinks_binary_and_limit_overflow(tmp_path: Path) -> None:
    ...
    cases = (
        (symlink_repository, ExecutionEvidenceFailureCode.UNSAFE_ENTRY),
        (binary_repository, ExecutionEvidenceFailureCode.INVALID_TEXT),
        (large_repository, ExecutionEvidenceFailureCode.FILE_TOO_LARGE),
        (total_repository, ExecutionEvidenceFailureCode.TOTAL_TOO_LARGE),
        (many_repository, ExecutionEvidenceFailureCode.TOO_MANY_FILES),
    )
    ...
    for repository, expected in cases:
        ...
        with pytest.raises(ExecutionEvidenceError) as caught:
