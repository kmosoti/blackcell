from __future__ import annotations

from dataclasses import replace
from datetime import UTC, datetime
from typing import cast

import pytest

from blackcell.gateway import DataClassification, GatewayBudget, LocalityPolicy
from blackcell.kernel._json import bytes_digest, canonical_json_bytes
from blackcell.orchestration.changes import (
    CHANGE_CONTEXT_SCHEMA,
    CHANGE_PROPOSAL_SCHEMA_VERSION,
    MAX_CHANGE_CONTEXT_BYTES,
    MAX_CHANGE_PROPOSAL_BYTES,
    ChangeContext,
    ChangeContractError,
    ChangeContractFailureCode,
    ChangeProposal,
    ChangeProviderCall,
    ChangeProviderResult,
    ExecutionEvidenceFile,
    FileChange,
    TextOperation,
    change_context_payload,
    change_proposal_from_mapping,
    change_proposal_payload,
    change_provider_result_payload,
)


def test_change_provider_wire_contracts_support_the_declared_evidence_budget() -> None:
    evidence_content = "x" * (256 * 1024)
    context = ChangeContext(
        objective="Use the complete admitted evidence budget.",
        constraints=("Preserve the declared behavior.",),
        base_commit="a" * 40,
        allowed_paths=("src",),
        max_changed_paths=4,
        files=tuple(
            _evidence_file(f"src/evidence-{index}.txt", evidence_content) for index in range(4)
        ),
    )
    proposal = ChangeProposal(
        proposal_id="proposal-large",
        evidence_digest=context.digest,
        operations=tuple(
            FileChange(
                TextOperation.CREATE,
                f"src/generated-{index}.txt",
                None,
                "x" * (1024 * 1024),
            )
            for index in range(4)
        ),
        summary="Create four contract-sized files.",
    )

    context_bytes = len(canonical_json_bytes(change_context_payload(context)))
    proposal_bytes = len(canonical_json_bytes(change_proposal_payload(proposal)))

    assert 1024 * 1024 < context_bytes <= MAX_CHANGE_CONTEXT_BYTES
    assert 1024 * 1024 < proposal_bytes <= MAX_CHANGE_PROPOSAL_BYTES


def test_evidence_context_is_bounded_content_addressed_and_authority_free() -> None:
    first = _evidence_file("src/a.py", "VALUE = 1\n")
    second = _evidence_file("README.md", "fixture\n")
    context = ChangeContext(
        objective="Add a bounded value.",
        constraints=("Do not alter public behavior.",),
        base_commit="a" * 40,
        allowed_paths=("src",),
        max_changed_paths=2,
        files=(first, second),
    )

    payload = change_context_payload(context)

    assert context.schema_version == CHANGE_CONTEXT_SCHEMA
    assert tuple(item.path for item in context.files) == ("README.md", "src/a.py")
    assert context.digest.startswith("sha256:") and len(context.digest) == 71
    assert payload["files"] == [
        {
            "path": "README.md",
            "content": "fixture\n",
            "content_digest": bytes_digest(b"fixture\n"),
        },
        {
            "path": "src/a.py",
            "content": "VALUE = 1\n",
            "content_digest": bytes_digest(b"VALUE = 1\n"),
        },
    ]
    serialized = repr(payload)
    assert "repository_root" not in serialized
    assert "worktree" not in serialized
    assert "executable" not in serialized
    assert "credential" not in serialized
    assert "network" not in serialized
    assert "executor" not in serialized
    assert "VALUE = 1" not in repr(first)

    with pytest.raises(ChangeContractError) as wrong_digest:
        ExecutionEvidenceFile("src/a.py", "VALUE = 1\n", "sha256:" + "0" * 64)
    assert wrong_digest.value.code is ChangeContractFailureCode.INVALID_EVIDENCE
    for path in (".git/config", "src/.git/config"):
        with pytest.raises(ChangeContractError) as metadata:
            _evidence_file(path, "forbidden\n")
        assert metadata.value.code is ChangeContractFailureCode.INVALID_EVIDENCE
    with pytest.raises(ChangeContractError) as oversized:
        _evidence_file("src/large.txt", "x" * (256 * 1024 + 1))
    assert oversized.value.code is ChangeContractFailureCode.INVALID_EVIDENCE
    with pytest.raises(ChangeContractError) as wrong_schema:
        replace(context, schema_version="change-context/v2")
    assert wrong_schema.value.code is ChangeContractFailureCode.INVALID_EVIDENCE


def test_change_proposal_parser_is_closed_and_binds_operation_semantics() -> None:
    old_digest = bytes_digest(b"old\n")
    mapping = {
        "schema_version": CHANGE_PROPOSAL_SCHEMA_VERSION,
        "proposal_id": "proposal-1",
        "evidence_digest": "sha256:" + "1" * 64,
        "operations": [
            {
                "operation": "replace",
                "path": "src/z.py",
                "expected_digest": old_digest,
                "content": "new\n",
            },
            {
                "operation": "create",
                "path": "src/a.py",
                "expected_digest": None,
                "content": "created\n",
            },
        ],
        "summary": "Create and replace two bounded files.",
    }

    proposal = change_proposal_from_mapping(mapping)

    assert proposal.schema_version == CHANGE_PROPOSAL_SCHEMA_VERSION
    assert tuple(item.path for item in proposal.operations) == ("src/a.py", "src/z.py")
    assert proposal.operations[0].operation is TextOperation.CREATE
    assert proposal.operations[0].content_digest == bytes_digest(b"created\n")
    assert proposal.operations[1].expected_digest == old_digest
    assert proposal.digest.startswith("sha256:")
    assert "created" not in repr(proposal.operations[0])

    invalid_values = (
        {**mapping, "unexpected": True},
        {**mapping, "operations": [{**mapping["operations"][0], "unexpected": True}]},
        {
            **mapping,
            "operations": [
                {
                    "operation": "create",
                    "path": ".git/config",
                    "expected_digest": None,
                    "content": "forbidden",
                }
            ],
        },
        {
            **mapping,
            "operations": [
                {
                    "operation": "create",
                    "path": "src/.git/config",
                    "expected_digest": None,
                    "content": "forbidden",
                }
            ],
        },
        {
            **mapping,
            "operations": [
                {
                    "operation": "replace",
                    "path": "src/a.py",
                    "expected_digest": None,
                    "content": "missing digest",
                }
            ],
        },
        {
            **mapping,
            "operations": [
                {
                    "operation": "delete",
                    "path": "src/a.py",
                    "expected_digest": old_digest,
                    "content": "not allowed",
                }
            ],
        },
        {
            **mapping,
            "operations": [
                {
                    "operation": "replace",
                    "path": "src/a.py",
                    "expected_digest": old_digest,
                    "content": "old\n",
                }
            ],
        },
        {
            **mapping,
            "operations": [
                mapping["operations"][0],
                mapping["operations"][0],
            ],
        },
    )
    for value in invalid_values:
        with pytest.raises(ChangeContractError) as caught:
            change_proposal_from_mapping(value)
        assert caught.value.code is ChangeContractFailureCode.INVALID_PROPOSAL

    with pytest.raises(ChangeContractError) as raw_operation:
        FileChange(
            "execute",  # ty: ignore[invalid-argument-type]
            "src/a.py",
            None,
            "content",
        )
    assert raw_operation.value.code is ChangeContractFailureCode.INVALID_PROPOSAL


def test_change_context_rejects_ambiguous_scope_and_unbounded_evidence() -> None:
    first = _evidence_file("src/a.py", "VALUE = 1\n")
    context = ChangeContext(
        objective="Apply one bounded change.",
        constraints=("Preserve behavior.",),
        base_commit="a" * 40,
        allowed_paths=("src",),
        max_changed_paths=2,
        files=(first,),
    )
    invalid_replacements = (
        {"objective": " \t"},
        {"constraints": cast("tuple[str, ...]", ["not-a-tuple"])},
        {"constraints": ("duplicate", "duplicate")},
        {"base_commit": "not-a-commit"},
        {"allowed_paths": cast("tuple[str, ...]", ["src"])},
        {"allowed_paths": ("src", "src")},
        {"max_changed_paths": True},
        {"max_changed_paths": -1},
        {"files": cast("tuple[ExecutionEvidenceFile, ...]", [first])},
        {"files": cast("tuple[ExecutionEvidenceFile, ...]", ("not-evidence",))},
        {"files": (first, first)},
    )
    for replacement in invalid_replacements:
        with pytest.raises(ChangeContractError) as caught:
            replace(context, **replacement)
        assert caught.value.code is ChangeContractFailureCode.INVALID_EVIDENCE

    large_files = tuple(
        _evidence_file(f"evidence/{index}.txt", "x" * (256 * 1024)) for index in range(5)
    )
    with pytest.raises(ChangeContractError) as aggregate_limit:
        replace(context, files=large_files)
    assert aggregate_limit.value.code is ChangeContractFailureCode.INVALID_EVIDENCE

    assert replace(context, allowed_paths=(".",)).allowed_paths == (".",)
    invalid_paths = ("", "/absolute", "src/../escape", "src\\value.py", "src/\x00value")
    for path in invalid_paths:
        with pytest.raises(ChangeContractError) as invalid_path:
            replace(context, allowed_paths=(path,))
        assert invalid_path.value.code is ChangeContractFailureCode.INVALID_EVIDENCE


def test_change_proposal_objects_enforce_bounded_inert_operations() -> None:
    digest = bytes_digest(b"old\n")
    valid = FileChange(TextOperation.REPLACE, "src/value.py", digest, "new\n")
    deleted = FileChange(TextOperation.DELETE, "src/old.py", digest, None)
    assert deleted.content_digest is None

    invalid_operations = (
        (TextOperation.CREATE, "src/value.py", digest, "new\n"),
        (TextOperation.CREATE, "src/value.py", None, None),
        (TextOperation.REPLACE, "src/value.py", None, "new\n"),
        (TextOperation.DELETE, "src/value.py", None, None),
        (TextOperation.DELETE, "src/value.py", digest, "content"),
        (TextOperation.CREATE, "src/value.py", None, "x" * (1024 * 1024 + 1)),
    )
    for operation, path, expected_digest, content in invalid_operations:
        with pytest.raises(ChangeContractError) as caught:
            FileChange(operation, path, expected_digest, content)
        assert caught.value.code is ChangeContractFailureCode.INVALID_PROPOSAL

    proposal = ChangeProposal(
        proposal_id="proposal-1",
        evidence_digest="sha256:" + "a" * 64,
        operations=(valid,),
        summary="Replace one bounded file.",
    )
    invalid_proposals = (
        {"schema_version": "change-proposal/v2"},
        {"proposal_id": "bad id"},
        {"evidence_digest": "not-a-digest"},
        {"operations": cast("tuple[FileChange, ...]", [])},
        {"operations": ()},
        {"operations": cast("tuple[FileChange, ...]", ("not-an-operation",))},
        {"operations": (valid, valid)},
        {"summary": ""},
        {"summary": "x" * 4097},
    )
    for replacement in invalid_proposals:
        with pytest.raises(ChangeContractError) as caught:
            replace(proposal, **replacement)
        assert caught.value.code is ChangeContractFailureCode.INVALID_PROPOSAL

    large_operations = tuple(
        FileChange(
            TextOperation.CREATE,
            f"src/generated-{index}.txt",
            None,
            "x" * (1024 * 1024),
        )
        for index in range(5)
    )
    with pytest.raises(ChangeContractError) as aggregate_limit:
        replace(proposal, operations=large_operations)
    assert aggregate_limit.value.code is ChangeContractFailureCode.INVALID_PROPOSAL


def test_change_provider_call_and_result_are_strict_content_free_boundaries() -> None:
    context = ChangeContext(
        objective="Apply one bounded change.",
        constraints=(),
        base_commit="a" * 40,
        allowed_paths=("src",),
        max_changed_paths=1,
        files=(_evidence_file("src/value.py", "VALUE = 1\n"),),
    )
    call = ChangeProviderCall(
        request_id="request-1",
        correlation_id="correlation-1",
        run_id="run-1",
        node_id="node-1",
        context=context,
        classification=DataClassification.PRIVATE,
        locality=LocalityPolicy.REMOTE_ALLOWED,
        budget=GatewayBudget(100, 50, 1_000, 0),
        estimated_input_tokens=10,
        causation_id="event-1",
    )
    assert call.context is context
    invalid_calls = (
        {"request_id": ""},
        {"correlation_id": "x" * 257},
        {"causation_id": " "},
        {"context": cast("ChangeContext", object())},
        {"classification": cast("DataClassification", "private")},
        {"locality": cast("LocalityPolicy", "remote-allowed")},
        {"budget": cast("GatewayBudget", object())},
        {"estimated_input_tokens": True},
        {"estimated_input_tokens": -1},
    )
    for replacement in invalid_calls:
        with pytest.raises(ChangeContractError) as caught:
            replace(call, **replacement)
        assert caught.value.code is ChangeContractFailureCode.INVALID_EVIDENCE

    proposal = ChangeProposal(
        proposal_id="proposal-1",
        evidence_digest=context.digest,
        operations=(FileChange(TextOperation.CREATE, "src/new.py", None, "NEW = 1\n"),),
        summary="Create one bounded file.",
    )
    result = ChangeProviderResult(
        proposal=proposal,
        provider_output_digest="sha256:" + "b" * 64,
        profile_id="execution",
        adapter_id="codex-cli",
        model_id="gpt-execution",
        input_tokens=10,
        output_tokens=5,
        latency_ms=100,
        cost_microusd=0,
        completed_at=datetime(2026, 7, 22, tzinfo=UTC),
    )
    assert change_provider_result_payload(result)["proposal_digest"] == proposal.digest
    invalid_results = (
        {"schema_version": "change-provider-result/v2"},
        {"proposal": cast("ChangeProposal", object())},
        {"provider_output_digest": "not-a-digest"},
        {"profile_id": ""},
        {"adapter_id": "x" * 257},
        {"input_tokens": True},
        {"output_tokens": -1},
        {"completed_at": datetime(2026, 7, 22)},
    )
    for replacement in invalid_results:
        with pytest.raises(ChangeContractError) as caught:
            replace(result, **replacement)
        assert caught.value.code is ChangeContractFailureCode.INVALID_PROPOSAL

    for serializer, value, code in (
        (change_context_payload, object(), ChangeContractFailureCode.INVALID_EVIDENCE),
        (change_proposal_payload, object(), ChangeContractFailureCode.INVALID_PROPOSAL),
        (
            change_provider_result_payload,
            object(),
            ChangeContractFailureCode.INVALID_PROPOSAL,
        ),
    ):
        with pytest.raises(ChangeContractError) as caught:
            serializer(value)  # ty: ignore[invalid-argument-type]
        assert caught.value.code is code


def test_change_proposal_parser_rejects_wrong_container_and_scalar_types() -> None:
    valid: dict[str, object] = {
        "schema_version": CHANGE_PROPOSAL_SCHEMA_VERSION,
        "proposal_id": "proposal-1",
        "evidence_digest": "sha256:" + "a" * 64,
        "operations": [
            {
                "operation": "create",
                "path": "src/new.py",
                "expected_digest": None,
                "content": "NEW = 1\n",
            }
        ],
        "summary": "Create one bounded file.",
    }
    variants: list[object] = [
        [],
        {**valid, "operations": "not-an-array"},
        {**valid, "operations": []},
        {**valid, "operations": ["not-an-object"]},
        {**valid, "operations": [{"operation": "create"}]},
    ]
    operation = cast("list[dict[str, object]]", valid["operations"])[0]
    variants.extend(
        (
            {**valid, "operations": [{**operation, "operation": "execute"}]},
            {**valid, "operations": [{**operation, "path": 1}]},
            {**valid, "operations": [{**operation, "expected_digest": 1}]},
            {**valid, "operations": [{**operation, "content": 1}]},
            {**valid, "summary": 1},
        )
    )
    for variant in variants:
        with pytest.raises(ChangeContractError) as caught:
            change_proposal_from_mapping(cast("dict[str, object]", variant))
        assert caught.value.code is ChangeContractFailureCode.INVALID_PROPOSAL


def _evidence_file(path: str, content: str) -> ExecutionEvidenceFile:
    return ExecutionEvidenceFile(path, content, bytes_digest(content.encode("utf-8")))
