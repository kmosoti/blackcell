...
TOKEN = "Runtime-review_config-token.0123456789-ABCDEFG"


def test_review_worker_config_loads_closed_owner_only_review_contract(
    tmp_path: Path,
) -> None:
    ...
    assert config.schema_version == REVIEW_CONFIG_SCHEMA
    assert config.source_path == source
    assert config.provider.profile_id == "review"
    ...
    assert config.worker.worker_id == "reviewer.test"
    assert config.worker.supervisor_id == "review-supervisor.test"
    assert config.worker.lease_seconds == 300
    assert config.worker.poll_milliseconds == 125
    assert load_review_config({}, repository_root=repository) is None


def test_review_config_rejects_provider_deadline_without_lease_reserve(
    tmp_path: Path,
) -> None:
    repository, _data_root, _isolation_root = _roots(tmp_path)
    for lease_seconds in (179, 180):
        ...
        with pytest.raises(ReviewConfigError) as caught:
            load_review_config(
                {REVIEW_CONFIG_FILE_ENV: str(source)},
                repository_root=repository,
            )

        assert str(caught.value) == "invalid-review-config"
...
def test_review_worker_config_rejects_unsafe_unknown_and_shared_authority(
    tmp_path: Path,
) -> None:
    ...
    duplicate = json.dumps(valid).replace(
        f'"schema_version": "{REVIEW_CONFIG_SCHEMA}"',
        f'"schema_version": "{REVIEW_CONFIG_SCHEMA}", "schema_version": "{REVIEW_CONFIG_SCHEMA}"',
        1,
    )
    ...
    for index, (source, content, mode) in enumerate(cases):
        ...
        assert str(caught.value) == "invalid-review-config", index
    ...
    cast("dict[str, object]", execution["provider"])["profile_id"] = "review"
    _write_config(execution_source, execution)
    environment = {
        DATA_DIR_ENV: str(data_root),
        API_TOKEN_ENV: TOKEN,
        REPOSITORY_ROOT_ENV: str(repository),
        EXECUTION_CONFIG_FILE_ENV: str(execution_source),
        REVIEW_CONFIG_FILE_ENV: str(review_source),
    }
    with pytest.raises(ProcessConfigError) as shared_profile:
        RuntimeProcessConfig.from_environment(environment)
    assert shared_profile.value.code is ProcessConfigFailureCode.INVALID_REVIEW_CONFIG

    cast("dict[str, object]", execution["provider"])["profile_id"] = "execution"
    cast("dict[str, object]", execution["worker"])["worker_id"] = "reviewer.test"
    _write_config(execution_source, execution)
    with pytest.raises(ProcessConfigError) as shared_worker:
        RuntimeProcessConfig.from_environment(environment)
    assert shared_worker.value.code is ProcessConfigFailureCode.INVALID_REVIEW_CONFIG
        ...
        RuntimeProcessConfig.from_environment(
            {
                ...
                API_TOKEN_ENV: TOKEN,
                REPOSITORY_ROOT_ENV: str(repository),
                REVIEW_CONFIG_FILE_ENV: str(unsafe_mode),
            }
        )
    assert unsafe_process.value.code is ProcessConfigFailureCode.INVALID_REVIEW_CONFIG


def test_review_config_rejects_each_typed_provider_boundary(tmp_path: Path) -> None:
    repository, _data_root, _isolation_root = _roots(tmp_path)
    valid = _review_payload()
    ...
    add("schema", None, "schema_version", "blackcell.review-config/v2")
    ...
    for index, (name, payload) in enumerate(variants):
        ...
        assert str(caught.value) == "invalid-review-config", name
    ...
    with pytest.raises(ReviewConfigError):
        load_review_config(
            {REVIEW_CONFIG_FILE_ENV: str((tmp_path / "missing-config.json").resolve())},
            repository_root=repository,
        )
    ...
    with pytest.raises(ReviewConfigError):
        load_review_config(
            {REVIEW_CONFIG_FILE_ENV: str(valid_source)},
